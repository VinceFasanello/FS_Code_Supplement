...
# Date: June 2021
# Project: Fluctuating selection project, Evolutionary Dynamics, counts processing
...
# proceeding.
# -------------------------------------
dir_home <- # enter the path to your "FS_Code_Supplement/Evolutionary_Dynamics" directory here. 
dir_source <- "/1_FastQtoCounts/R_Source"
dir_metadata <- "/1_FastQtoCounts/Evolutionary_Dynamics/Metadata"
dir_it_fastq <- "/1_FastQtoCounts/Evolutionary_Dynamics/IonTorrent_Files/IonTorrent_FastQ"
dir_it_df <- "/1_FastQtoCounts/Evolutionary_Dynamics/IonTorrent_Files/IonTorrent_DF"
dir_counts <-"/1_FastQtoCounts/Evolutionary_Dynamics/Counts"
dir_counts2 <- "/2_CountstoAnalysis/Counts_Data"
# ------------------------------------------------------------------------------
...
mobyBCs <- read.csv(file = "MobyBCs.csv", header = TRUE, stringsAsFactors = FALSE)
...
primers <- read.csv(file = "Primers.csv", header = TRUE, stringsAsFactors = FALSE)
# ------------------------------------------------------------------------------
...
p1.primer.ids <- primers$primer.id[25:32]		# Store the names of the reverse primers in an object
p1.primer.bcs <- primers$primer.barcode.r[25:32]	# stores the sequence of the reverse primers in an object
...
# completely with zeros except in the 
# cells where counts are expected 
# based on the experimental design.
# 
# "counts.unexpected" is the opposite,
# it contains zeros in all cells where
