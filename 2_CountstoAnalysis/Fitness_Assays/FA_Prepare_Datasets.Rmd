...
uc1 <- counts.uxpected; rm(counts.uxpected)
uc1 <- uc1 + 1 # add one pseudocount to all cells
...
ec2 <- ec2 + 1 # add one pseudocount to all cells
ec2 <- t(ec2) # transpose to match metadata format: experiments are rows

load(file = "Counts_Unexpected.Rdata") # unexpected counts (barcode cross contamination)
uc2 <- counts.uxpected; rm(counts.uxpected)
uc2 <- uc2 + 1 # add one pseudocount to all cells
