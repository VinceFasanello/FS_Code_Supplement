...
Convert the one-pp-per-row dataset to a dataset format with data for a single well (at a single timepoint) per row.
```{R}
my <- myd 
...
concatonate such that the data for a single well at all timepoints is on a single row. 
```{R}
my <- myd
my <- my[order(my$experiment.id),]
...
my$fixed[my$diff0 <= st & my$diff49 >= ft] <- TRUE # calculated fixed
...
my[my$bc1 %in% EH0, c("treat", "min", "max")] <- c(rep("EH0", times = nrow(my[my$bc1 %in% EH0,])),
                                                   ...
                                                   rep(0, times = nrow(my[my$bc1 %in% EH0,])))
# 20% stress, Uniform
EH20 <- c("d1A2", "d2A2", "d1B2", "d2B2", "d1C2", "d2C2", "d1D2", "d2D2",
          "d1E2", "d2E2", "d1F2", "d2F2", "d1G2", "d2G2", "d1H2", "d2H2")
my[my$bc1 %in% EH20, c("treat", "min", "max")] <- c(rep("EH20", times = nrow(my[my$bc1 %in% EH20,])),
                                                   rep(20, times = nrow(my[my$bc1 %in% EH20,])),
                                                   rep(20, times = nrow(my[my$bc1 %in% EH20,])))
# 40% stress, Uniform
...
my[my$bc1 %in% EH40, c("treat", "min", "max")] <- c(rep("EH40", times = nrow(my[my$bc1 %in% EH40,])),
                                                   ...
                                                   rep(40, times = nrow(my[my$bc1 %in% EH40,])))
# 60% stress, Uniform
EH60 <- c("d1A4", "d2A4", "d1B4", "d2B4", "d1C4", "d2C4", "d1D4", "d2D4",
          "d1E4", "d2E4", "d1F4", "d2F4", "d1G4", "d2G4", "d1H4", "d2H4")
my[my$bc1 %in% EH60, c("treat", "min", "max")] <- c(rep("EH60", times = nrow(my[my$bc1 %in% EH60,])),
                                                   rep(60, times = nrow(my[my$bc1 %in% EH60,])),
                                                   rep(60, times = nrow(my[my$bc1 %in% EH60,])))
# 80% stress, Uniform
...
my[my$bc1 %in% EH80, c("treat", "min", "max")] <- c(rep("EH80", times = nrow(my[my$bc1 %in% EH80,])),
                                                   ...
                                                   rep(80, times = nrow(my[my$bc1 %in% EH80,])))
# 100% stress, Uniform
EH100 <- c("d1A6", "d2A6", "d1B6", "d2B6", "d1C6", "d2C6", "d1D6", "d2D6",
          "d1E6", "d2E6", "d1F6", "d2F6", "d1G6", "d2G6", "d1H6", "d2H6")
my[my$bc1 %in% EH100, c("treat", "min", "max")] <- c(rep("EH100", times = nrow(my[my$bc1 %in% EH100,])),
                                                   rep(100, times = nrow(my[my$bc1 %in% EH100,])),
                                                   rep(100, times = nrow(my[my$bc1 %in% EH100,])))
...
# 0/60% stress, Fluctuating
EH0.60 <- c("d1A8", "d2A8", "d1B8", "d2B8", "d1C8", "d2C8", "d1D8", "d2D8",
            "d1E8", "d2E8", "d1F8", "d2F8", "d1G8", "d2G8", "d1H8", "d2H8")
my[my$bc1 %in% EH0.60, c("treat", "min", "max")] <- c(rep("EH0_60", times = nrow(my[my$bc1 %in% EH0.60,])),
                                                   rep(0, times = nrow(my[my$bc1 %in% EH0.60,])),
                                                   rep(60, times = nrow(my[my$bc1 %in% EH0.60,])))
...
myd <- my
rm(EH0, EH20, EH40, EH60, EH80, EH100, EH0.40, EH0.60, EH0.80, EH20.60, EH40.80, my)
```

REMOVE ALL PROBLEM ENTRIES
```{R}
my <- myd
# remove rows with bc1 id == "d2C5" and "d2C9", these barcodes share a barcode sequence and therefore cannot be analyzed.
my <- my[my$bc1 != "d1C5" & my$bc1 != "d2C5",]
my <- my[my$bc1 != "d1C9" & my$bc1 != "d2C9",]
# these other wells have one lineage with an unrecovered barcode -- need to omit as well. 
my <- my[my$bc1 != "d1C4" & my$bc1 != "d2C4",]
my <- my[my$bc1 != "d1D8" & my$bc1 != "d2D8",]
my <- my[my$bc1 != "d1E8" & my$bc1 != "d2E8",]

myd <- my # pass back to myfa
rm(my)
```

APPEND FIT ASSAY FIT VALUES - append fitness assay values here if desired. Not currently implemented. 
```{R}
...
setwd("..")
setwd(dir_out_f)
myd$treat <- factor(myd$treat, levels = c("EH0", "EH20", "EH0_40", "EH0_60", "EH40", "EH20_60", "EH0_80", "EH60", "EH40_80", "EH80", "EH100"))
save(myd, file = "myd.Rdata") # save the myevo dataframe for downstream use.
```