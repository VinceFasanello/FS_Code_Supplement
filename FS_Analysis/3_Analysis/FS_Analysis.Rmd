...
wtd.cor(mu$dw_00, mu$dw_40, weight = log(mu$reads_dw_00_40), bootse = T)

mu <- mt[!is.na(mt$dw_00) &  !is.na(mt$dw_80),]
# cor.test(mu$dw_00, mu$dw_80)
wtd.cor(mu$dw_00, mu$dw_80, weight = log(mu$reads_dw_00_80), bootse = T)

mu <- mt[!is.na(mt$dw_00) &  !is.na(mt$dw_120),]
# cor.test(mu$dw_00, mu$dw_120)
wtd.cor(mu$dw_00, mu$dw_120, weight = log(mu$reads_dw_00_120), bootse = T)
...
wtd.cor(mu$dw_40, mu$dw_80, weight = log(mu$reads_dw_40_80), bootse = T)

mu <- mt[!is.na(mt$dw_40) &  !is.na(mt$dw_120),]
# cor.test(mu$dw_40, mu$dw_120)
wtd.cor(mu$dw_40, mu$dw_120, weight = log(mu$reads_dw_40_120), bootse = T)
...
mu <- mt[!is.na(mt$dw_80) &  !is.na(mt$dw_120),]
# cor.test(mu$dw_80, mu$dw_120)
wtd.cor(mu$dw_80, mu$dw_120, weight = log(mu$reads_dw_80_120), bootse = T)
...
EH0in00 <- weighted.mean(m$dw_00, m$reads_dw_00, na.rm = T)
EH0in40 <- weighted.mean(m$dw_40, m$reads_dw_40, na.rm = T)
EH0in80 <- weighted.mean(m$dw_80, m$reads_dw_80, na.rm = T)

m <- myrcw[myrcw$treat == "EH40" & myrcw$epo %in% c("SALT_A", "SALT_B"),]
EH40in00 <- weighted.mean(m$dw_00, m$reads_dw_00, na.rm = T)
EH40in40 <- weighted.mean(m$dw_40, m$reads_dw_40, na.rm = T)
EH40in80 <- weighted.mean(m$dw_80, m$reads_dw_80, na.rm = T)

m <- myrcw[myrcw$treat == "EH80" & myrcw$epo %in% c("SALT_A", "SALT_B"),]
EH80in00 <- weighted.mean(m$dw_00, m$reads_dw_00, na.rm = T)
EH80in40 <- weighted.mean(m$dw_40, m$reads_dw_40, na.rm = T)
EH80in80 <- weighted.mean(m$dw_80, m$reads_dw_80, na.rm = T)


EH0_00_40 <- c(EH0in00, EH0in40)
EH0_00_80 <- c(EH0in00, EH0in80)

EH40_00_40 <- c(EH40in00, EH40in40)
EH40_40_80 <- c(EH40in40, EH40in80)

EH80_00_80 <- c(EH80in00, EH80in80)
EH80_40_80 <- c(EH80in40, EH80in80)
...
EH0_80in00 <- weighted.mean(m$dw_00, m$reads_dw_00, na.rm = T)
EH0_80in80 <- weighted.mean(m$dw_80, m$reads_dw_80, na.rm = T)

EH0_80_00_80 <- c(EH0_80in00, EH0_80in80)

m <- myrcw[myrcw$treat == "EH0_40" & myrcw$epo %in% c("SALT_A", "SALT_B"),]
EH0_40in00 <- weighted.mean(m$dw_00, m$reads_dw_00, na.rm = T)
EH0_40in40 <- weighted.mean(m$dw_40, m$reads_dw_40, na.rm = T)

EH0_40_00_40 <- c(EH0_40in00, EH0_40in40)
