...
```{R CC, warning=FALSE}
# NOTE: replace ccpt with ccpm in all reportvals[n] lines to report mean contaminatn rather than total cc (IF DESIRED)
...
# # Save outputs to file -------------------------------------------------------
# setwd(DIR_out); write.csv(mycc, file = "000_Table_Sx_cc.csv")
...
for (g in 1:length(chems)){
  for (h in 1:length(envs)){
    ...
    for(i in 1:length(treatlist)){
      ...
      #                 file=paste0("000_Table_Sn_",strsplit(chems[[g]][1], "_")[[1]][1],"_",envs[h], ".html"))) # save individual models if desired
...
for (i in 1:length(chems)){
  for (j in 1:length(envs)){
      ...
      print(tab_model(mod, digits=5, digits.p=8, show.stat=T, wrap.labels=100,
                      file=paste0("000_Table_Sn_",strsplit(chems[[i]][1], "_")[[1]][1],"_",envs[j], ".html")))  # save individual models if desired
  }
...
```{R cost association with fit change}
# get data for plotting --------------------------------------------------------
m1 <- myrcw[myrcw$dw_00 > 0 & myrcw$dw_80 < 0,]
m1 <- m1[m1$treat == "EH0",]
m2 <- myrcw[myrcw$dw_00 < 0 & myrcw$dw_80 > 0,]
m2 <- m2[m2$treat == "EH80",]
m12 <- rbind(m1, m2); rm(m1, m2)


# plot for NaCl data, EH0 EH80 -------------------------------------------------
m <- m12[m12$treat %in%c( "EH0", "EH80") & m12$epo %in% c("SALT_A", "SALT_B"),]
ggplot(data = m, aes(x=dw_00, y = dw_80, color = treat, fill = treat))+
  geom_point() + 
  scale_color_manual(values = mypalette[c(1,7)])+ 
  scale_fill_manual(values = mypalette[c(1,7)])+ 
  theme_VJF() + 
...
# correlation tests --------------------
ms <- m[m$epo %in% c("SALT_A", "SALT_B"),]
ms0 <- ms[ms$treat == "EH0",]
cor.test(ms0$dw_00, ms0$dw_80, na.action(na.omit))
ms80 <- ms[ms$treat == "EH80",]
cor.test(ms80$dw_00, ms80$dw_80, na.action(na.omit))
rm(ms, ms0, ms80, m)


# plot for CuSO4 data, EH0 EH80 ------------------------------------------------
m <- m12[m12$treat %in%c( "EH0", "EH80") & m12$epo %in% c("COPR_A", "COPR_B"),]
ggplot(data = m, aes(x=dw_00, y = dw_80, color = treat, fill = treat))+
  geom_point() + 
  scale_color_manual(values = mypalette[c(1,7)])+ 
  scale_fill_manual(values = mypalette[c(1,7)])+ 
...
# correlation tests --------------------
mc <- m[m$epo %in% c("COPR_A", "COPR_B"),]
# mc0 <- mc[mc$treat == "EH0",]
# cor.test(mc0$dw_00, mc0$dw_80, na.action(na.omit)) # only one observation. 
mc80 <- mc[mc$treat == "EH80",]
cor.test(mc80$dw_00, mc80$dw_80, na.action(na.omit))
rm(mc, mc0, mc80, m)
...
# version including EH0_80 -----------------------------------------------------
# NOTE: THERE ARE NO CASES OF INC IN 0 and DEC in 80 for EH0_80
# prep data ----------------------------
m <- myrcw[myrcw$dw_00 > 0 & myrcw$dw_80 < 0,]
m1 <- m[which(m$treat == "EH0"),]
m31 <- m[which(m$treat == "EH0_80"),]
# m31$treat <- "EH0_80_i0"
rm(m)

m <- myrcw[myrcw$dw_00 < 0 & myrcw$dw_80 > 0,]
m2 <- m[which(m$treat == "EH80"),]
m32 <- m[which(m$treat == "EH0_80"),]
m32$treat <- "EH0_80_i80"
rm(m)

# plot NaCl ----------------------------
m <- rbind(m1, m2, m31, m32)
m <- m[m$treat %in%c( "EH0", "EH80", "EH0_80_i0", "EH0_80_i80") & m$epo %in% c("SALT_A", "SALT_B"),]
ggplot(data = m, aes(x=dw_00, y = dw_80, color = treat, fill = treat))+
  geom_point() + 
  # scale_color_manual(values = mypalette[c(1,7)])+ 
  # scale_fill_manual(values = mypalette[c(1,7)])+ 
...
# plot CuSO4 ---------------------------
m <- rbind(m1, m2, m31, m32)
m <- m[m$treat %in%c( "EH0", "EH80", "EH0_80_i0", "EH0_80_i80") & m$epo %in% c("COPR_A", "COPR_B"),]
ggplot(data = m, aes(x=dw_00, y = dw_80, color = treat, fill = treat))+
  geom_point() + 
  # scale_color_manual(values = mypalette[c(1,7)])+ 
  # scale_fill_manual(values = mypalette[c(1,7)])+ 
...
# ------------------------------------------------------------------------------
# OPTION 1: Plot all data in a single panel (a little dense and hard to digest) 
# p1 <- ggplot()+
...
summary(mymod)
print(tab_model(mymod, digits=5, digits.p=8, show.stat=T, wrap.labels=100,
...
png("000_Figure_3.png", width = 6, height =6, units = "in",res = 300)
...
png("000_Figure_S3.png", width = 11/1.25, height =8.5/1.25, units = "in",res = 300)
grid.arrange(p2,p1,p4,zp2,zp1,zp4, ncol = 3)
dev.off()
...
for (g in 1:length(chems)){
  ...
  for(i in 1:length(treatlist)){
    ...
    # print(tab_model(mod, digits = 5, digits.p = 8, show.stat = T, wrap.labels = 100,
    #                 file = paste0("000_Table_Sm_",strsplit(chems[[g]][1], "_")[[1]][1], ".html"))) # print table results for each model if desired
    for(j in 1:length(treattreatlist)){
      if(treatlist[j] != treatlist[i]){
...
for (i in 1:length(chems)){
  ...
  print(tab_model(mod, digits = 5, digits.p = 8, show.stat = T, wrap.labels = 100,
                  file = paste0("000_Table_Sm_",strsplit(chems[[i]][1], "_")[[1]][1], ".html"))) # print table results for each model if desired
}; rm(chems, m, mod, i)
```
<br/><br/>
...
m1 %>%
  group_by(treat) %>%
  summarise(dec = sum(dw_00_80 <= -0.0216),
            nod = sum(dw_00_80 > -0.0216 & dw_00_80 < 0.0216),
            inc = sum(dw_00_80 >= 0.0216),
            tot = length(dw_00_80))
...
# Set plotting controls --------------------------------------------------------
fitaxdatlimMN <- c(-0.1*100, 0.35*100) #; min(myrcw$dw_00_80, na.rm = T); max(myrcw$dw_00_80, na.rm=T)
fitaxlablimMN <- c(-0.1*100, 0.35*100) 
boxMN <- c(0.165*100, 0.36*100) #; (boxMN[2]-boxMN[1]) / (fitaxdatlimMN[2] - fitaxdatlimMN[1])
treatlabSALT <- "NaCl Evol. Treatment"
treatlabCOPR <- expression(bold(CuSO["4"]*"  Evol. Treatment"))
fitnesslab1 <- "Geometric Mean Fitness Change (%); NaCl dataset"
fitnesslab2 <- expression(bold("Geometric Mean Fitness Change (%); "*CuSO["4"]*" dataset"))
treatlist <- c("EH0","EH0_40","EH40","EH20_60","EH0_80","EH40_80","EH80")
...
rm(pAA, pBB, melt_myres_backer, melt_myres_est_mn_COPR, melt_myres_est_mn_SALT,
   m1, m2, treatlabCOPR, treatlabSALT, fitnesslab1, fitnesslab2, boxMN,  fitaxlablimMN,
   m, fitaxdatlimMN) # tidy
```
<br/><br/>


```{R Geometric Mean fitness}
# NaCl
...
summary(mymod)
getwd()
tab_model(mymod, digits = 5, digits.p = 8, show.stat = T, wrap.labels = 100, show.r2 = F, file = "000_Table_S10.html")
rm(m, mymod)
# Additional Models ------------------------------------------------------------
...
summary(mymod)
getwd()
tab_model(mymod, digits = 5, digits.p = 8, show.stat = T, wrap.labels = 100, show.r2 = F, file = "000_Table_S11.html")
rm(m, mymod)
...
x <- cbind(x[1:(nrow(x)/2),], x[((nrow(x)/2)+1):nrow(x),])[,c(1,2,4,5)];x; rm(m,x)
...
# fishers exact test for fixation rate. ----------------------------------------
# build dataframe by hand from above calculations !!!(replace this with more elgant code when time allows)!!!
?fisher.test
TeaTasting <-
matrix(c(3, 1, 1, 3),
       nrow = 2,
...
saltfix <- matrix(c(9+8,((16+14)-(9+8)), 4+3+5+5, ((16+16+14+16)-(4+3+5+5))), nrow = 2, byrow = T)
saltfix
mymod <- chisq.test(saltfix, simulate.p.value = F, correct = T)
mymod$expected
mymod

coprfix <- matrix(c(6+5,((8+8)-(6+5)), 10+4+7+8, ((16+16+14+16)-(10+4+7+8))), nrow = 2, byrow = T)
coprfix
mymod <- chisq.test(coprfix, simulate.p.value = F, correct = T)
mymod
mymod$expected
```
<br/><br/>
...
```{R Clonal Interference}
# scrape Legend ----------------------------------------------------------------
m <- myrcw[myrcw$treat %in% c("EH0_80", "EH0_40", "EH40_80", "EH20_60"),]
legp <- ggplot(data = m, aes( x = dw_00, y = dw_80, color = treat))+
  geom_point(size = 2) + theme_VJF() + scale_color_manual(values = mypalette[c(2,4,5,6)]) + labs(color = "Treatment") + guides(color = guide_legend(reverse = T))+
  theme(legend.title = element_text(size = 8), 
          legend.text = element_text(size = 8))
legp <- get_legend(legp); rm(m)
...
# NaCl -------------------------------------------------------------------------
# data ---------------------------------
m <- myrcw[myrcw$treat %in% c("EH0_80", "EH0_40", "EH40_80", "EH20_60") & myrcw$epo %in% c("SALT_A", "SALT_B"),]
m$platewell <- paste0(m$epo, m$well)
m <- m[m$platewell %ni% c("SALT_Ad1C9d2C9", "SALT_Bd1C9d2C9"),]
m <- m[order(m$platewell),]
length(unique(m$platewell))
m1 <- m[c(seq(1, 61, 2)),]
m2 <- m[c(seq(2, 62, 2)),]
sum(m1$platewell != m2$platewell)

# viz ---------------------------------
mm <- as.data.frame(cbind(m1$dw_00, m2$dw_00, as.character(m1$treat))); colnames(mm) <- c("Barcode_1", "Barcode_2", "Treatment")
mm$Treatment <- factor(mm$Treatment, levels = c("EH0_40", "EH20_60", "EH0_80", "EH40_80"))
mm$Barcode_1 <- as.numeric(mm$Barcode_1)
mm$Barcode_2 <- as.numeric(mm$Barcode_2)
mm <- mm[complete.cases(mm),]
p1 <- ggplot(data=mm, aes(x=Barcode_1, y=Barcode_2))+
  geom_smooth(method='lm', color = "black")+
  geom_point( aes(color=Treatment), size = 3)+
  scale_color_manual(values = mypalette[c(2,4,5,6)])+
  scale_fill_manual(values = mypalette[c(2,4,5,6)])+
  theme_VJF() + theme(legend.position="none")+
  geom_hline(yintercept = 0, lty = 1) + geom_hline(yintercept = 0.0217, lty = 2) + geom_hline(yintercept = -0.0217, lty = 2) +
  geom_vline(xintercept = 0, lty = 1) + geom_vline(xintercept = 0.0217, lty = 2) + geom_vline(xintercept = -0.0217, lty = 2) +
  ggtitle("NaCl Dataset, Well Fitness 0% NaCl") + labs(tag="A") + xlab("")+ ylab("Barcode 1 Fitness Change")

# viz ---------------------------------
mm <- as.data.frame(cbind(m1$dw_80, m2$dw_80, as.character(m1$treat))); colnames(mm) <- c("Barcode_1", "Barcode_2", "Treatment")
mm$Treatment <- factor(mm$Treatment, levels = c("EH0_40", "EH20_60", "EH0_80", "EH40_80"))
mm$Barcode_1 <- as.numeric(mm$Barcode_1)
mm$Barcode_2 <- as.numeric(mm$Barcode_2)
mm <- mm[complete.cases(mm),]
p2 <- ggplot(data=mm, aes(x=Barcode_1, y=Barcode_2))+
  geom_smooth(method='lm', color = "black")+
  geom_point( aes(color=Treatment), size = 3)+
  scale_color_manual(values = mypalette[c(2,4,5,6)])+
  scale_fill_manual(values = mypalette[c(2,4,5,6)])+
  ...
  geom_hline(yintercept = 0, lty = 1) + geom_hline(yintercept = 0.0217, lty = 2) + geom_hline(yintercept = -0.0217, lty = 2) +
  geom_vline(xintercept = 0, lty = 1) + geom_vline(xintercept = 0.0217, lty = 2) + geom_vline(xintercept = -0.0217, lty = 2) +
  ggtitle("NaCl Dataset, Well Fitness 80% NaCl") + labs(tag="C") + xlab("Barcode 1 Fitness Change")+ ylab("Barcode 2 Fitness Change")


# CuSO4 -------------------------------------------------------------------------
# data ---------------------------------
m <- myrcw[myrcw$treat %in% c("EH0_80", "EH0_40", "EH40_80", "EH20_60") & myrcw$epo %in% c("COPR_A", "COPR_B"),]
m$platewell <- paste0(m$epo, m$well)
m <- m[m$platewell %ni% c("COPR_Ad1C9d2C9", "COPR_Bd1C9d2C9"),]
m <- m[order(m$platewell),]
length(unique(m$platewell))
m1 <- m[c(seq(1, 61, 2)),]
m2 <- m[c(seq(2, 62, 2)),]
...
mm$Barcode_2 <- as.numeric(mm$Barcode_2)
mm <- mm[complete.cases(mm),]
p3 <- ggplot(data=mm, aes(x=Barcode_1, y=Barcode_2))+
  geom_smooth(method='lm', color = "black")+
  geom_point( aes(color=Treatment), size = 3)+
  scale_color_manual(values = mypalette[c(2,4,5,6)])+
  scale_fill_manual(values = mypalette[c(2,4,5,6)])+
  ...
  geom_hline(yintercept = 0, lty = 1) + geom_hline(yintercept = 0.0217, lty = 2) + geom_hline(yintercept = -0.0217, lty = 2) +
  geom_vline(xintercept = 0, lty = 1) + geom_vline(xintercept = 0.0217, lty = 2) + geom_vline(xintercept = -0.0217, lty = 2) +
  ggtitle("CuSO4 Dataset, Well Fitness 0% CuSO4") + labs(tag="B") + xlab("")+ ylab("")
...
mm$Barcode_2 <- as.numeric(mm$Barcode_2)
mm <- mm[complete.cases(mm),]
p4 <- ggplot(data=mm, aes(x=Barcode_1, y=Barcode_2))+
  geom_smooth(method='lm', color = "black")+
  geom_point( aes(color=Treatment), size = 3)+
  scale_color_manual(values = mypalette[c(2,4,5,6)])+
  scale_fill_manual(values = mypalette[c(2,4,5,6)])+
  ...
  geom_hline(yintercept = 0, lty = 1) + geom_hline(yintercept = 0.0217, lty = 2) + geom_hline(yintercept = -0.0217, lty = 2) +
  geom_vline(xintercept = 0, lty = 1) + geom_vline(xintercept = 0.0217, lty = 2) + geom_vline(xintercept = -0.0217, lty = 2) +
  ggtitle("CuSO4 Dataset, Well Fitness 80% CuSO4") + annotation_custom(legp, xmin = 0.2, ymin = -0.35) + labs(tag="D") + xlab("Barcode 1 Fitness Change")+ ylab("")

# produce test figure ----------------------------------------------------------
pdf("000_Figure_test.pdf", width = 11/1.25, height =8.5/1.25)
grid.arrange(p1,p3,p2,p4, ncol = 2)
dev.off()

png("000_Figure_test.png", width = 3300, height =2550, res = 300)
grid.arrange(p1,p3,p2,p4, ncol = 2)
dev.off()
rm(legp, m, m1, m2, mm, p1, p2, p3, p4)
```
<br/><br/>
