...
# version including EH0_80 (THERE ARE NO CASES OF INC IN 0 and DEC in 80 for EH0_80)!!!
...
m <- m[!is.na(m$dw_00),]
m <- m[m$dw_00 >= 0.0216,]
nrow(m[which(m$dw_80 <= -0.0216),]) + nrow(m[is.na(m$dw_80),])


m <- myrcw[myrcw$epo %in% c("SALT_A", "SALT_B") & myrcw$treat %in% c("EH80"),]
m <- m[!is.na(m$dw_80),]
m <- m[m$dw_80 >= 0.0216,]
nrow(m[which(m$dw_00 <= -0.0216),]) + nrow(m[is.na(m$dw_00),])
...
m <- m[!is.na(m$dw_00),]
m <- m[m$dw_00 >= 0.0216,]
nrow(m[which(m$dw_40 <= -0.0216),]) + nrow(m[is.na(m$dw_40),])
nrow(m[which(m$dw_80 <= -0.0216),]) + nrow(m[is.na(m$dw_80),])
...
# fishers exact test for fixation rate. build dataframe by hand from above calculations (replace this with more elgant code when possible)
?fisher.test
TeaTasting <-
matrix(c(3, 1, 1, 3),
       nrow = 2,
