...
# report -----------------------------------------------------------------------
for (i in 1:10){print(reportvals[i])}
...
powerTR$v  <- c(rep((28*2) -1 -1, times=100), rep((10*2) -1 -1, times=100)) # v's
powerTR$u <- 1 # u
powerTR$n <- powerTR$v + 2; powerTR$n <- factor(powerTR$n, levels=c(56,20)) # n=v+2
...
reportvals[3] <- paste0(pwr.f2.test(u=1, v=((28*2) -1 - 1), f2=myeffectsize, sig.level=0.05)$power*100, "% power to detect a 1% fitness change between treatments (Best Case)")
reportvals[4] <- paste0("80% power to detect a ", pwr.f2.test(u=1, v=((28*2) -1 - 1), power=0.8, sig.level=0.05)$f2*myrmse, "% fitness change between treatments (Best Case)")
reportvals[5] <- paste0(pwr.f2.test(u=1, v=((10*2) -1 - 1), f2=myeffectsize, sig.level=0.05)$power*100, "% power to detect a 1% fitness difference between treatments (Worst Case)")
reportvals[6] <- paste0("80% power to detect a ", pwr.f2.test(u=1, v=((10*2) -1 - 1), power=0.8, sig.level=0.05)$f2*myrmse, "% fitness change for an average BC (Worst Case)")
...
pBB<-ggplot(powerTR, aes(x=f2, y=power, group=n)) +
  geom_segment(x=pwr.f2.test(u=1, v=((28*2) -1 - 1), power=0.8, sig.level=0.05)$f2*myrmse,
               xend=pwr.f2.test(u=1, v=((28*2) -1 - 1), power=0.8, sig.level=0.05)$f2*myrmse, y=0, yend=0.8, lty="dotdash", color="gray", size=0.25)+
  geom_segment(x=pwr.f2.test(u=1, v=((10*2) -1 - 1), power=0.8, sig.level=0.05)$f2*myrmse,
               xend=pwr.f2.test(u=1, v=((10*2) -1 - 1), power=0.8, sig.level=0.05)$f2*myrmse, y=0, yend=0.8, lty="dotdash", color="gray", size=0.25)+
  ...
  geom_text(x=0.2, y=0.85, color="darkgray", size=2, label=paste0(round(pwr.f2.test(u=1, v=((28*2) -1 - 1), power=0.8, sig.level=0.05)$f2*myrmse, 2),", 80"))+
  geom_text(x=1.22, y=0.75, color="darkgray", size=2, label=paste0(round(pwr.f2.test(u=1, v=((10*2) -1 - 1), power=0.8, sig.level=0.05)$f2*myrmse, 2),", 80"))+
  geom_line(aes(lty=n))+
...
setwd(DIR_out); pdf(file = paste("000_Figure_2.pdf"),height = 6, width = 11)
...
# Save outputs to file ---------------------------------------------------------
setwd(DIR_out); pdf(file = paste("000_Figure_2_COPR.pdf"),height = 6, width = 11)
grid::grid.draw(grid.arrange(p1,p2,p3,p4,p5,p6,p7,p8,p1leg, layout_matrix =lay)); dev.off()
