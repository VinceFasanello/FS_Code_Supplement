...
pdf("000_Figure_3ac.pdf", width = 3, height = 6)
...
lay<- as.matrix(cbind(c(1,1,2)))
pdf("000_Figure_3bd.pdf", width = 3, height = 6)
grid::grid.draw(grid.arrange(p1,p2, layout_matrix =lay))
dev.off()
```
...
pdf("000_Figure_2.pdf", width = 4, height = 5)
p1
dev.off()
```
...
for (i in c("SALT", "COPR")) {
  for (j in c("mn")) {
    ...
    assign(paste0("melt_myres_est_", j, "_", i), melt_myres_est_temp)
      ...
      myres_p_mn_COPR, myres_p_mn_SALT, get_lower_tri)
...
# Save outputs to file ---------------------------------------------------------
setwd(DIR_out); pdf(file = "000_Figure_S3.pdf", width = 11/2, height = 8.5)
cowplot::plot_grid(pAA, pBB, nrow = 2); dev.off()
rm(pAA, pBB, melt_myres_backer, melt_myres_est_mn_COPR, melt_myres_est_mn_SALT,
    treatlabCOPR, treatlabSALT,
   fitnesslab1, fitnesslab2, boxMN,  fitaxlablimMN, viopanel)
```
<br/><br/>
...
pdf("000_Figure_S3abc.pdf", width = 7, height =2)
...
pdf("000_Figure_S3def.pdf", width = 7, height =2)
grid.arrange(p2,p1,p4, ncol = 3)
dev.off()
```