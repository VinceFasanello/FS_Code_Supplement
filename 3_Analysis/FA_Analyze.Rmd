...
# SALT evolved in CM
# Plot: violin plots; treatment effect on dw
```{r}
# data -------------------------------------------------------------------------
...
# models: lmer dw~treat, random effect of bcpID to account for data structure (replicate measures of fitness)
```{r}
# MODELS -----------------------------------------------------------------------
my <- myd[!is.na(myd$dw),] 
my <- my[my$mpamt == "CM" & my$mpasp == 0.0 & my$epo %in% c("SALT_A", "SALT_B"),]
...
x <- table(my$treat)/4; myCOPRtot[,c(1:4)] <- x[c(1, 5, 7, 2, 4, 3, 6)]
my <- my[!is.na(my$dw),]
...
Set 1: 0, 0_80, 80
```{R}
my <- mydwcw[mydwcw$treat %in% c("EH0", "EH0_80", "EH80") & mydwcw$epo %in% c("SALT_A", "SALT_B"),]
...
div_ext <- cbind(mySALTtot["EH0",1] - nrow(mynot[mynot$treat == "EH0",]),
             ...
             mySALTtot["EH80",1] - nrow(mynot[mynot$treat == "EH80",])); names(div_ext) <- c("EH0", "EH0_80", "EH80")
div_tot <- mySALTtot[c("EH0", "EH0_80", "EH80"),1]


# how many increase in both environments
x <- my[my$in0 <= 0.05 & my$in80 <= 0.05,]
# x <- my[my$mn_dw_0 > 0 & my$mn_dw_80 > 0,]
r1 <- round(summary(x$treat), digits = 3); r1 <- r1[c("EH0", "EH0_80", "EH80")]

# how many increase in the .00 but not .80 env
x <- my[my$in0 <= 0.05 & my$in80 > 0.05,]
# x <- my[my$mn_dw_0 <= 0 & my$mn_dw_80 > 0,]
r2 <- round(summary(x$treat), digits = 3); r2 <- r2[c("EH0", "EH0_80", "EH80")]

# how many increase in the .80 but not .00 env
x <- my[my$in0 > 0.05 & my$in80 <= 0.05,]
# x <- my[my$mn_dw_0 > 0 & my$mn_dw_80 <= 0,]
r3 <- round(summary(x$treat), digits = 3); r3 <- r3[c("EH0", "EH0_80", "EH80")]

# how many increase in neither the .80 nor .00 env
x <- my[my$in0 > 0.05 & my$in80 > 0.05,]
# x <- my[my$mn_dw_0 <= 0 & my$mn_dw_80 <= 0,]
r4 <- round(summary(x$treat), digits = 3); r4 <- r4[c("EH0", "EH0_80", "EH80")]

mymat <- rbind(r1, r2, r3, r4, ext, div_tot)
colnames(mymat) <- c("EH0", "EH0_80", "EH80")
...
write.csv(mymat,  file = "table1_SALT_s_g_fs_inc_raw.csv")
write.csv(mymat2, file = "table1_SALT_s_g_fs_inc_divtot.csv")
write.csv(mymat3, file = "table1_SALT_s_g_fs_inc_divext.csv")
```
...
mymat <- rbind(r1, r2, r3, r4, ext, div_tot)
colnames(mymat) <- c("EH0", "EH0_80", "EH80")
rownames(mymat) <- c("dec_both", "dec_0", "dec_80", "dec_neither", "n_extinct", "n_tot")
mymat
...
write.csv(mymat,  file = "table1_SALT_s_g_fs_dec_raw.csv")
write.csv(mymat2, file = "table1_SALT_s_g_fs_dec_divtot.csv")
write.csv(mymat3, file = "table1_SALT_s_g_fs_dec_divext.csv")
```
...
Set 1: 0, 0_80, 80
```{R}
my <- mydwcw[mydwcw$treat %in% c("EH0", "EH0_80", "EH80") & mydwcw$epo %in% c("COPR_A", "COPR_B"),]
my$treat <- drop.levels(my$treat)
mynot <- my[is.na(my$in0) | is.na(my$in80),]
my <- my[!is.na(my$in0) & !is.na(my$in80),]
ext <- c(nrow(mynot[mynot$treat == "EH0",]),
...
div_ext <- cbind(myCOPRtot["EH0",1] - nrow(mynot[mynot$treat == "EH0",]),
             ...
             myCOPRtot["EH80",1] - nrow(mynot[mynot$treat == "EH80",])); names(div_ext) <- c("EH0", "EH0_80", "EH80")
div_tot <- myCOPRtot[c("EH0", "EH0_80", "EH80"),1]
...
write.csv(mymat,  file = "table1_COPR_s_g_fs_inc_raw.csv")
write.csv(mymat2, file = "table1_COPR_s_g_fs_inc_divtot.csv")
write.csv(mymat3, file = "table1_COPR_s_g_fs_inc_divext.csv")
```
...
write.csv(mymat,  file = "table1_COPR_s_g_fs_dec_raw.csv")
write.csv(mymat2, file = "table1_COPR_s_g_fs_dec_divtot.csv")
write.csv(mymat3, file = "table1_COPR_s_g_fs_dec_divext.csv")
```
...
# Take a look at extinction in without any stats tests.*, **
...
Now there are many many tedious comparisons that must be drawn between the treatments in different environments....
this will be 17 tests X 4 environments X 2 variables X 2 chemicals = 272 tests....
These tests are computed here and the p-values are stored in a set of matrices (n=16; 2 chemicals x 2 vars x 4 envs )
* It is important to note that these are not ALL possible pairwise comparisons, but only those of particular interest!
# SALT -------------------------------------------------------------------------
# INCREASE -------------------------------------------------
```{r}
mytdata <- mySALTin
my <- matrix(nrow =2, ncol=2)
myres <- matrix(nrow=7, ncol=7); colnames(myres) <- rownames(mySALTin); rownames(myres) <- colnames(myres)

# 0.0 --------------------------------------------
# 0 vs ---------------------------------
my[1,1] <- mytdata["EH0", "0"]
my[1,2] <- mySALTtot["EH0","0"] - my[1,1]
# 40 ---------------
my[2,1] <- mytdata["EH40","0"]
my[2,2] <- mySALTtot["EH40","0"] - my[2,1]; # my
myres["EH0", "EH40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 80 ---------------
my[2,1] <- mytdata["EH80","0"]
my[2,2] <- mySALTtot["EH80","0"] - my[2,1]; # my
myres["EH0", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","0"]
my[2,2] <- mySALTtot["EH0_40","0"] - my[2,1]; # my
myres["EH0", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0"]
my[2,2] <- mySALTtot["EH0_80","0"] - my[2,1]; # my
myres["EH0", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 40 vs ---------------------------------
my[1,1] <- mytdata["EH40", "0"]
my[1,2] <- mySALTtot["EH40","0"] - my[1,1]
# 80 ---------------
my[2,1] <- mytdata["EH80","0"]
my[2,2] <- mySALTtot["EH80","0"] - my[2,1]; # my
myres["EH40", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","0"]
my[2,2] <- mySALTtot["EH0_40","0"] - my[2,1]; # my
myres["EH40", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","0"]
my[2,2] <- mySALTtot["EH20_60","0"] - my[2,1]; # my
myres["EH40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0"]
my[2,2] <- mySALTtot["EH0_80","0"] - my[2,1]; # my
myres["EH40", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- mySALTtot["EH40_80","0"] - my[2,1]; # my
myres["EH40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 80 vs ---------------------------------
my[1,1] <- mytdata["EH80", "0"]
my[1,2] <- mySALTtot["EH80","0"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- mySALTtot["EH40_80","0"] - my[2,1]; # my
myres["EH80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0"]
my[2,2] <- mySALTtot["EH0_80","0"] - my[2,1]; # my
myres["EH80", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 0_40 vs ---------------------------------
my[1,1] <- mytdata["EH0_40", "0"]
my[1,2] <- mySALTtot["EH0_40","0"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- mySALTtot["EH40_80","0"] - my[2,1]; # my
myres["EH0_40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0"]
my[2,2] <- mySALTtot["EH0_80","0"] - my[2,1]; # my
myres["EH0_40", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","0"]
my[2,2] <- mySALTtot["EH20_60","0"] - my[2,1]; # my
myres["EH0_40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 20_60 vs ---------------------------------
my[1,1] <- mytdata["EH20_60", "0"]
my[1,2] <- mySALTtot["EH20_60","0"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- mySALTtot["EH40_80","0"] - my[2,1]; # my
myres["EH20_60", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0"]
my[2,2] <- mySALTtot["EH0_80","0"] - my[2,1]; # my
myres["EH20_60", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 0_80 vs ---------------------------------
my[1,1] <- mytdata["EH0_80", "0"]
my[1,2] <- mySALTtot["EH0_80","0"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- mySALTtot["EH40_80","0"] - my[2,1]; # my
myres["EH0_80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value

# save results -----
p_SALT_inc_000 <- myres
...
# 0.4 --------------------------------------------
# 0 vs ---------------------------------
my[1,1] <- mytdata["EH0", "0.4"]
my[1,2] <- mySALTtot["EH0","0.4"] - my[1,1]
# 40 ---------------
my[2,1] <- mytdata["EH40","0.4"]
my[2,2] <- mySALTtot["EH40","0.4"] - my[2,1]; # my
myres["EH0", "EH40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 80 ---------------
my[2,1] <- mytdata["EH80","0.4"]
my[2,2] <- mySALTtot["EH80","0.4"] - my[2,1]; # my
myres["EH0", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","0.4"]
my[2,2] <- mySALTtot["EH0_40","0.4"] - my[2,1]; # my
myres["EH0", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0.4"]
my[2,2] <- mySALTtot["EH0_80","0.4"] - my[2,1]; # my
myres["EH0", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 40 vs ---------------------------------
my[1,1] <- mytdata["EH40", "0.4"]
my[1,2] <- mySALTtot["EH40","0.4"] - my[1,1]
# 80 ---------------
my[2,1] <- mytdata["EH80","0.4"]
my[2,2] <- mySALTtot["EH80","0.4"] - my[2,1]; # my
myres["EH40", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
myres["EH40", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","0.4"]
my[2,2] <- mySALTtot["EH20_60","0.4"] - my[2,1]; # my
myres["EH40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0.4"]
my[2,2] <- mySALTtot["EH0_80","0.4"] - my[2,1]; # my
myres["EH40", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- mySALTtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 80 vs ---------------------------------
my[1,1] <- mytdata["EH80", "0.4"]
my[1,2] <- mySALTtot["EH80","0.4"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- mySALTtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_40 vs ---------------------------------
my[1,1] <- mytdata["EH0_40", "0.4"]
my[1,2] <- mySALTtot["EH0_40","0.4"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- mySALTtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH0_40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 vs ---------------------------------
my[1,1] <- mytdata["EH20_60", "0.4"]
my[1,2] <- mySALTtot["EH20_60","0.4"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- mySALTtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH20_60", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_80 vs ---------------------------------
my[1,1] <- mytdata["EH0_80", "0.4"]
my[1,2] <- mySALTtot["EH0_80","0.4"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- mySALTtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH0_80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0.8 --------------------------------------------
# 0 vs ---------------------------------
my[1,1] <- mytdata["EH0", "0.8"]
my[1,2] <- mySALTtot["EH0","0.8"] - my[1,1]
# 40 ---------------
my[2,1] <- mytdata["EH40","0.8"]
my[2,2] <- mySALTtot["EH40","0.8"] - my[2,1]; # my
myres["EH0", "EH40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 80 ---------------
my[2,1] <- mytdata["EH80","0.8"]
my[2,2] <- mySALTtot["EH80","0.8"] - my[2,1]; # my
myres["EH0", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","0.8"]
my[2,2] <- mySALTtot["EH0_40","0.8"] - my[2,1]; # my
myres["EH0", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0.8"]
my[2,2] <- mySALTtot["EH0_80","0.8"] - my[2,1]; # my
myres["EH0", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 40 vs ---------------------------------
my[1,1] <- mytdata["EH40", "0.8"]
my[1,2] <- mySALTtot["EH40","0.8"] - my[1,1]
# 80 ---------------
my[2,1] <- mytdata["EH80","0.8"]
my[2,2] <- mySALTtot["EH80","0.8"] - my[2,1]; # my
myres["EH40", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
myres["EH40", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","0.8"]
my[2,2] <- mySALTtot["EH20_60","0.8"] - my[2,1]; # my
myres["EH40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0.8"]
my[2,2] <- mySALTtot["EH0_80","0.8"] - my[2,1]; # my
myres["EH40", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- mySALTtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 80 vs ---------------------------------
my[1,1] <- mytdata["EH80", "0.8"]
my[1,2] <- mySALTtot["EH80","0.8"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- mySALTtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_40 vs ---------------------------------
my[1,1] <- mytdata["EH0_40", "0.8"]
my[1,2] <- mySALTtot["EH0_40","0.8"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- mySALTtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH0_40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 vs ---------------------------------
my[1,1] <- mytdata["EH20_60", "0.8"]
my[1,2] <- mySALTtot["EH20_60","0.8"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- mySALTtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH20_60", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_80 vs ---------------------------------
my[1,1] <- mytdata["EH0_80", "0.8"]
my[1,2] <- mySALTtot["EH0_80","0.8"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- mySALTtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH0_80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 1.2 --------------------------------------------
# 0 vs ---------------------------------
my[1,1] <- mytdata["EH0", "1.2"]
my[1,2] <- mySALTtot["EH0","1.2"] - my[1,1]
# 40 ---------------
my[2,1] <- mytdata["EH40","1.2"]
my[2,2] <- mySALTtot["EH40","1.2"] - my[2,1]; # my
myres["EH0", "EH40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 80 ---------------
my[2,1] <- mytdata["EH80","1.2"]
my[2,2] <- mySALTtot["EH80","1.2"] - my[2,1]; # my
myres["EH0", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","1.2"]
my[2,2] <- mySALTtot["EH0_40","1.2"] - my[2,1]; # my
myres["EH0", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","1.2"]
my[2,2] <- mySALTtot["EH0_80","1.2"] - my[2,1]; # my
myres["EH0", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 40 vs ---------------------------------
my[1,1] <- mytdata["EH40", "1.2"]
my[1,2] <- mySALTtot["EH40","1.2"] - my[1,1]
# 80 ---------------
my[2,1] <- mytdata["EH80","1.2"]
my[2,2] <- mySALTtot["EH80","1.2"] - my[2,1]; # my
myres["EH40", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
myres["EH40", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","1.2"]
my[2,2] <- mySALTtot["EH20_60","1.2"] - my[2,1]; # my
myres["EH40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","1.2"]
my[2,2] <- mySALTtot["EH0_80","1.2"] - my[2,1]; # my
myres["EH40", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- mySALTtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 80 vs ---------------------------------
my[1,1] <- mytdata["EH80", "1.2"]
my[1,2] <- mySALTtot["EH80","1.2"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- mySALTtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_40 vs ---------------------------------
my[1,1] <- mytdata["EH0_40", "1.2"]
my[1,2] <- mySALTtot["EH0_40","1.2"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- mySALTtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH0_40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 vs ---------------------------------
my[1,1] <- mytdata["EH20_60", "1.2"]
my[1,2] <- mySALTtot["EH20_60","1.2"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- mySALTtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH20_60", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_80 vs ---------------------------------
my[1,1] <- mytdata["EH0_80", "1.2"]
my[1,2] <- mySALTtot["EH0_80","1.2"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- mySALTtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH0_80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# save results -----
p_SALT_inc_120 <- myres

# ----------
p_SALT_inc_000[is.nan(p_SALT_inc_000)] <- 1
p_SALT_inc_040[is.nan(p_SALT_inc_040)] <- 1
p_SALT_inc_080[is.nan(p_SALT_inc_080)] <- 1
p_SALT_inc_120[is.nan(p_SALT_inc_120)] <- 1
rm(my, myres, mytdata)
```
<br/><br/>
...
# save results -----
p_SALT_dec_120 <- myres

# ----------
p_SALT_dec_000[is.nan(p_SALT_dec_000)] <- 1
p_SALT_dec_040[is.nan(p_SALT_dec_040)] <- 1
p_SALT_dec_080[is.nan(p_SALT_dec_080)] <- 1
p_SALT_dec_120[is.nan(p_SALT_dec_120)] <- 1
rm(my, myres, mytdata)
```
<br/><br/>
...
mytdata <- myCOPRin
my <- matrix(nrow =2, ncol=2)
myres <- matrix(nrow=7, ncol=7); colnames(myres) <- rownames(myCOPRin); rownames(myres) <- colnames(myres)

# 0.0 --------------------------------------------
# 0 vs ---------------------------------
my[1,1] <- mytdata["EH0", "0"]
my[1,2] <- myCOPRtot["EH0","0"] - my[1,1]
# 40 ---------------
my[2,1] <- mytdata["EH40","0"]
my[2,2] <- myCOPRtot["EH40","0"] - my[2,1]; # my
myres["EH0", "EH40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 80 ---------------
my[2,1] <- mytdata["EH80","0"]
my[2,2] <- myCOPRtot["EH80","0"] - my[2,1]; # my
myres["EH0", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","0"]
my[2,2] <- myCOPRtot["EH0_40","0"] - my[2,1]; # my
myres["EH0", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0"]
my[2,2] <- myCOPRtot["EH0_80","0"] - my[2,1]; # my
myres["EH0", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 40 vs ---------------------------------
my[1,1] <- mytdata["EH40", "0"]
my[1,2] <- myCOPRtot["EH40","0"] - my[1,1]
# 80 ---------------
my[2,1] <- mytdata["EH80","0"]
my[2,2] <- myCOPRtot["EH80","0"] - my[2,1]; # my
myres["EH40", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","0"]
my[2,2] <- myCOPRtot["EH20_60","0"] - my[2,1]; # my
myres["EH40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0"]
my[2,2] <- myCOPRtot["EH0_80","0"] - my[2,1]; # my
...
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- myCOPRtot["EH40_80","0"] - my[2,1]; # my
myres["EH40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 80 vs ---------------------------------
my[1,1] <- mytdata["EH80", "0"]
my[1,2] <- myCOPRtot["EH80","0"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- myCOPRtot["EH40_80","0"] - my[2,1]; # my
myres["EH80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_40 vs ---------------------------------
my[1,1] <- mytdata["EH0_40", "0"]
my[1,2] <- myCOPRtot["EH0_40","0"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- myCOPRtot["EH40_80","0"] - my[2,1]; # my
myres["EH0_40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 vs ---------------------------------
my[1,1] <- mytdata["EH20_60", "0"]
my[1,2] <- myCOPRtot["EH20_60","0"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- myCOPRtot["EH40_80","0"] - my[2,1]; # my
myres["EH20_60", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_80 vs ---------------------------------
my[1,1] <- mytdata["EH0_80", "0"]
my[1,2] <- myCOPRtot["EH0_80","0"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0"]
my[2,2] <- myCOPRtot["EH40_80","0"] - my[2,1]; # my
myres["EH0_80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0 vs ---------------------------------
my[1,1] <- mytdata["EH0", "0.4"]
my[1,2] <- myCOPRtot["EH0","0.4"] - my[1,1]
# 40 ---------------
my[2,1] <- mytdata["EH40","0.4"]
my[2,2] <- myCOPRtot["EH40","0.4"] - my[2,1]; # my
myres["EH0", "EH40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 80 ---------------
my[2,1] <- mytdata["EH80","0.4"]
my[2,2] <- myCOPRtot["EH80","0.4"] - my[2,1]; # my
myres["EH0", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","0.4"]
my[2,2] <- myCOPRtot["EH0_40","0.4"] - my[2,1]; # my
myres["EH0", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0.4"]
my[2,2] <- myCOPRtot["EH0_80","0.4"] - my[2,1]; # my
myres["EH0", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 40 vs ---------------------------------
my[1,1] <- mytdata["EH40", "0.4"]
my[1,2] <- myCOPRtot["EH40","0.4"] - my[1,1]
# 80 ---------------
my[2,1] <- mytdata["EH80","0.4"]
my[2,2] <- myCOPRtot["EH80","0.4"] - my[2,1]; # my
myres["EH40", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","0.4"]
my[2,2] <- myCOPRtot["EH20_60","0.4"] - my[2,1]; # my
myres["EH40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0.4"]
my[2,2] <- myCOPRtot["EH0_80","0.4"] - my[2,1]; # my
...
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- myCOPRtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 80 vs ---------------------------------
my[1,1] <- mytdata["EH80", "0.4"]
my[1,2] <- myCOPRtot["EH80","0.4"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- myCOPRtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_40 vs ---------------------------------
my[1,1] <- mytdata["EH0_40", "0.4"]
my[1,2] <- myCOPRtot["EH0_40","0.4"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- myCOPRtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH0_40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 vs ---------------------------------
my[1,1] <- mytdata["EH20_60", "0.4"]
my[1,2] <- myCOPRtot["EH20_60","0.4"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- myCOPRtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH20_60", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_80 vs ---------------------------------
my[1,1] <- mytdata["EH0_80", "0.4"]
my[1,2] <- myCOPRtot["EH0_80","0.4"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.4"]
my[2,2] <- myCOPRtot["EH40_80","0.4"] - my[2,1]; # my
myres["EH0_80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0 vs ---------------------------------
my[1,1] <- mytdata["EH0", "0.8"]
my[1,2] <- myCOPRtot["EH0","0.8"] - my[1,1]
# 40 ---------------
my[2,1] <- mytdata["EH40","0.8"]
my[2,2] <- myCOPRtot["EH40","0.8"] - my[2,1]; # my
myres["EH0", "EH40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 80 ---------------
my[2,1] <- mytdata["EH80","0.8"]
my[2,2] <- myCOPRtot["EH80","0.8"] - my[2,1]; # my
myres["EH0", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","0.8"]
my[2,2] <- myCOPRtot["EH0_40","0.8"] - my[2,1]; # my
myres["EH0", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0.8"]
my[2,2] <- myCOPRtot["EH0_80","0.8"] - my[2,1]; # my
myres["EH0", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 40 vs ---------------------------------
my[1,1] <- mytdata["EH40", "0.8"]
my[1,2] <- myCOPRtot["EH40","0.8"] - my[1,1]
# 80 ---------------
my[2,1] <- mytdata["EH80","0.8"]
my[2,2] <- myCOPRtot["EH80","0.8"] - my[2,1]; # my
myres["EH40", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","0.8"]
my[2,2] <- myCOPRtot["EH20_60","0.8"] - my[2,1]; # my
myres["EH40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","0.8"]
my[2,2] <- myCOPRtot["EH0_80","0.8"] - my[2,1]; # my
...
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- myCOPRtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 80 vs ---------------------------------
my[1,1] <- mytdata["EH80", "0.8"]
my[1,2] <- myCOPRtot["EH80","0.8"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- myCOPRtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_40 vs ---------------------------------
my[1,1] <- mytdata["EH0_40", "0.8"]
my[1,2] <- myCOPRtot["EH0_40","0.8"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- myCOPRtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH0_40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 vs ---------------------------------
my[1,1] <- mytdata["EH20_60", "0.8"]
my[1,2] <- myCOPRtot["EH20_60","0.8"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- myCOPRtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH20_60", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_80 vs ---------------------------------
my[1,1] <- mytdata["EH0_80", "0.8"]
my[1,2] <- myCOPRtot["EH0_80","0.8"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","0.8"]
my[2,2] <- myCOPRtot["EH40_80","0.8"] - my[2,1]; # my
myres["EH0_80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0 vs ---------------------------------
my[1,1] <- mytdata["EH0", "1.2"]
my[1,2] <- myCOPRtot["EH0","1.2"] - my[1,1]
# 40 ---------------
my[2,1] <- mytdata["EH40","1.2"]
my[2,2] <- myCOPRtot["EH40","1.2"] - my[2,1]; # my
myres["EH0", "EH40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 80 ---------------
my[2,1] <- mytdata["EH80","1.2"]
my[2,2] <- myCOPRtot["EH80","1.2"] - my[2,1]; # my
myres["EH0", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_40 -------------
my[2,1] <- mytdata["EH0_40","1.2"]
my[2,2] <- myCOPRtot["EH0_40","1.2"] - my[2,1]; # my
myres["EH0", "EH0_40"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","1.2"]
my[2,2] <- myCOPRtot["EH0_80","1.2"] - my[2,1]; # my
myres["EH0", "EH0_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 40 vs ---------------------------------
my[1,1] <- mytdata["EH40", "1.2"]
my[1,2] <- myCOPRtot["EH40","1.2"] - my[1,1]
# 80 ---------------
my[2,1] <- mytdata["EH80","1.2"]
my[2,2] <- myCOPRtot["EH80","1.2"] - my[2,1]; # my
myres["EH40", "EH80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 -------------
my[2,1] <- mytdata["EH20_60","1.2"]
my[2,2] <- myCOPRtot["EH20_60","1.2"] - my[2,1]; # my
myres["EH40", "EH20_60"] <- chisq.test(x = my, simulate.p.value = T)$p.value
# 0_80 -------------
my[2,1] <- mytdata["EH0_80","1.2"]
my[2,2] <- myCOPRtot["EH0_80","1.2"] - my[2,1]; # my
...
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- myCOPRtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value


# 80 vs ---------------------------------
my[1,1] <- mytdata["EH80", "1.2"]
my[1,2] <- myCOPRtot["EH80","1.2"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- myCOPRtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_40 vs ---------------------------------
my[1,1] <- mytdata["EH0_40", "1.2"]
my[1,2] <- myCOPRtot["EH0_40","1.2"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- myCOPRtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH0_40", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 20_60 vs ---------------------------------
my[1,1] <- mytdata["EH20_60", "1.2"]
my[1,2] <- myCOPRtot["EH20_60","1.2"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- myCOPRtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH20_60", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# 0_80 vs ---------------------------------
my[1,1] <- mytdata["EH0_80", "1.2"]
my[1,2] <- myCOPRtot["EH0_80","1.2"] - my[1,1]
# 40_80 -------------
my[2,1] <- mytdata["EH40_80","1.2"]
my[2,2] <- myCOPRtot["EH40_80","1.2"] - my[2,1]; # my
myres["EH0_80", "EH40_80"] <- chisq.test(x = my, simulate.p.value = T)$p.value
...
# save results -----
p_COPR_inc_120 <- myres

# ----------
p_COPR_inc_000[is.nan(p_COPR_inc_000)] <- 1
p_COPR_inc_040[is.nan(p_COPR_inc_040)] <- 1
p_COPR_inc_080[is.nan(p_COPR_inc_080)] <- 1
p_COPR_inc_120[is.nan(p_COPR_inc_120)] <- 1
rm(my, myres, mytdata)
```
<br/><br/>
...
# save results -----
p_COPR_dec_120 <- myres

# ----------
p_COPR_dec_000[is.nan(p_COPR_dec_000)] <- 1
p_COPR_dec_040[is.nan(p_COPR_dec_040)] <- 1
p_COPR_dec_080[is.nan(p_COPR_dec_080)] <- 1
p_COPR_dec_120[is.nan(p_COPR_dec_120)] <- 1
rm(my, myres, mytdata)
```
...
# INCREASE_v2 -------------------------------------------------
```{r}
myCOPRtot_holder <- myCOPRtot
myCOPRtot <- myCOPRtot - myCOPRext
...
# save results -----
p_COPR_inc_120_v2 <- myres

# ----------
p_COPR_inc_000_v2[is.nan(p_COPR_inc_000_v2)] <- 1
p_COPR_inc_040_v2[is.nan(p_COPR_inc_040_v2)] <- 1
p_COPR_inc_080_v2[is.nan(p_COPR_inc_080_v2)] <- 1
p_COPR_inc_120_v2[is.nan(p_COPR_inc_120_v2)] <- 1
myCOPRtot <- myCOPRtot_holder
rm(my, myres, mytdata)
```
<br/><br/>