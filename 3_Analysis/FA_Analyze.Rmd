...
my <- my[my$epo %in% c("SALT_A", "SALT_B") & my$mpasp == 0.0 & !is.na(my$dw),]
...
# ----------------------------
mySALTnleft <- mySALTno  # give a clearer name. 
rm(my, mymod, myres, tmp, i, mysig, mySALTno)
```
<br/><br/>
...
# COPR in CM -------------------------------------------------------------------
my <- myd[order(myd$bcpID),] 
my <- my[my$epo %in% c("COPR_A", "COPR_B") & my$mpasp == 0.0 & !is.na(my$dw),]
...
# ----------------------------
myCOPRnleft <- myCOPRno  # give a clearer name. 
rm(my, mymod, myres, tmp, i, mysig, myCOPRno)
```
<br/><br/>
...
# ------------------
rm(my, my1, my2, my3, my4, mn_dw, mn_re_dw)
```
...
my <- mydwc[mydwc$epo %in% c("SALT_A", "SALT_B") & mydwc$mpasp == 0.0 & mydwc$treat %in% c("EH0","EH0_40","EH40","EH20_60","EH0_80", "EH40_80", "EH80"),]
my$treat <- factor(my$treat, levels = c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"))
...
p
pdf(file = "000_rank_SALTinCM.pdf", width = mywidth, height = myheight); p; dev.off()
...
my <- mydwc[mydwc$epo %in% c("COPR_A", "COPR_B") & mydwc$mpasp == 0.0 & mydwc$treat %in% c("EH0","EH0_40","EH40","EH20_60","EH0_80", "EH40_80", "EH80"),]
my$treat <- factor(my$treat, levels = c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"))
my <- my[!is.na(my$mn_dw),]
my <- my[order(my$mn_dw),]
p<-ggplot(data=my, aes(x=reorder(bcpID, -mn_dw), y=mn_dw, fill = treat)) +
  geom_bar (stat="identity")
p
pdf(file = "000_rank_COPRinCM.pdf", width = mywidth, height = myheight); p; dev.off()
...
mn_re_dw <- apply(my, 1, function(x) mean(as.numeric(x[c( "dw", "dw_2", "dw_3", "dw_4")])), na.rm = T)
