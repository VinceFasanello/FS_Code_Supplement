...
my$diff_0 <- abs(my$mn_dw - my$mn_dw_bc2); my$diff_0 <- scale(my$diff_0, center = F)
my$diff_80 <- abs(my$mn_dw_80 - my$mn_dw_bc2_80); my$diff_80 <- scale(my$diff_80, center = F)

hull_treat <- my[!is.na(my$diff_0) & !is.na(my$diff_80),]  %>%
  group_by(treat) %>%
  slice(chull(diff_0, diff_80))
...
q <- ggplot(my, aes(x= disto,color= treat)) +
