...
mymod1 <- dredge(mymod, fixed = c("my$treat")) # weight of 0.907 on full model, with next highest weight being 0.068, only retain full model (model averaging will fail here! with a delta AIC <=4). 
...
mymod1 <- dredge(mymod, fixed = c("my$treat")) 
...
mymod1 <- dredge(mymod, fixed = c("my$treat")) # weight of 0.989 on full model, with next highest weight being 0.006, only retain full model (model averaging will fail here! with a delta <=4). 
...
my$treat <- relevel(my$treat, ref = "EH0")
mymod <- lmer(my$dw ~ my$treat + I(scale(my$fit0)) + I(scale(my$medcount)) + I(scale(my$medprop)) + (1|my$bcpID) + 1, weights = my$re_dw, control=lmerControl(optimizer ="bobyqa",optCtrl=list(maxfun=2e4)), na.action = "na.fail");# summary(mymod)
car::vif(mymod) # VIF looks okay, proceed to model reduction
AIC(mymod)
