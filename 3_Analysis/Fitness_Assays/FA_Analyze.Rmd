...
ggplot(myt, aes(x=value, color = treat, lty = variable))+
...
myt$wdn[(nrow(myt)/2+1):nrow(myt)] <- NA
myt <- reshape::melt(myt)
ggplot(myt, aes(x=value, lty = variable))+
  geom_density(alpha = 0.1, trim = mytrim)+
  xlim(min(myt$value, na.rm = T), max(myt$value, na.rm = T))+
  theme_tufte(base_family="Helvetica")
...
myt$wdn[(nrow(myt)/2+1):nrow(myt)] <- NA
myt <- reshape::melt(myt)
ggplot(myt, aes(x=value,color = group, lty = variable))+
  geom_density(alpha = 0.1, trim = mytrim)+
  xlim(min(myt$value, na.rm = T), max(myt$value, na.rm = T))+
  theme_tufte(base_family="Helvetica")
...
# once settled on which plots to use --- will need to create for all environments -- NOW JUST IN 00 (NEED 40, 80, 120) -- code is flexible, control block at top of block. 
# need to do outputs too for all plots!
```
