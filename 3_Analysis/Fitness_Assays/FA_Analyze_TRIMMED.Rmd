...
MN VS VAR FITNESS (2 ENVIRONMENT) -- TARGET GROUPS (TEMPLATE CODE -- SCRAPE FROM HERE AND USE ABOVE WHEN DETAILS ARE FIGURED OUT!!!!)
```{r}
...
Kassen Plot -- geometric????
```{r}
my <- myfit500cw[mydwcw$epo %in% c("SALT_A", "SALT_B"),]
my$Treatment <- factor(my$treat, levels = c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80")) # reorder the treat factor
my$id <- paste0(my$epo, my$bc1) # assign bcids
myf <- my # save a frame for limits
my$Fitness_in_CM <- my$mn_fit500_0 # store fitness in complete media
my$Fitness_in_High_Stress <- my$mn_fit500_80 # store fitness in stress
...
myp <- my
myp$cutoff <- NA; myp$cutoff_elip <- NA
myp$md <- NA
for (i in c("EH0", "EH0_80","EH80")) {
  ...
  cutoff_elip <- (qchisq(p = 1 - alpha_elip, df = ncol(myp[myp$Treatment == i,][, c("Fitness_in_CM", "Fitness_in_High_Stress")])))
...
points(myp[myp$md >= myp$cutoff_elip,]$Fitness_in_High_Stress ~ myp[myp$md >= myp$cutoff_elip,]$Fitness_in_CM, col = "blue")
...
centroids <- aggregate(cbind(mypc$Fitness_in_CM, mypc$Fitness_in_High_Stress)~mypc$Treatment,mypc,psych::geometric.mean)
colnames(centroids) <-  c("Treatment","Fitness_in_CM", "Fitness_in_High_Stress")
# get dataframe for elipses ------
mypc_elip <- myp[myp$md < myp$cutoff_elip,]


# work on geometric fitness lines
geomfit <- matrix(nrow= 200, ncol = 200)
x <- seq(from = 0.7,to = 1.5, length.out = 200)
y <- seq(from = 0.7,to = 1.5, length.out = 200)
colnames(geomfit) <- x
rownames(geomfit) <- y
...
p1 <- ggplot()+
  ...
  xlim( min(c(myf$mn_fit500_0, myf$mn_fit500_40), na.rm = T), max(c(myf$mn_fit500_0, myf$mn_fit500_40), na.rm = T))+
  ylim( min(c(myf$mn_fit500_40, myf$mn_fit500_80), na.rm = T), max(c(myf$mn_fit500_40, myf$mn_fit500_80), na.rm = T))+
  ...
  annotate("text", x = c(0.89, 0.9, 0.93, 0.95, 1.025), y = c(1.125, 1.25, 1.33, 1.4, 1.425), label = c("w = 1", "w = 1.05", "w = 1.1", "w = 1.15", "w = 1.2"))+

  stat_ellipse(data = mypc_elip[mypc_elip$treat %in% c("EH0","EH0_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, fill = Treatment, color = Treatment), alpha = 0.15, geom = "polygon", linetype = 2, show.legend = F)+
  ...
  geom_point(data = myp[myp$treat %in% c("EH0","EH0_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress), shape = 1, colour = "black", size = 2.5, alpha = 0.7, show.legend = F)+
  geom_segment(mapping = aes(x=centroids[1,2], y=centroids[1,3], xend=centroids[3,2], yend=centroids[3,3]), linetype=1, size=1.5, inherit.aes=F)+
  geom_point(data=centroids, aes(x=Fitness_in_CM, y=Fitness_in_High_Stress), color = "black", cex = 5, shape = 21, fill = "black")+
  theme_bw()+
  ...
  scale_fill_manual(values = mypalette)
p1
pdf(file = paste("000_SALT_KASSEN_0_80_wlines.pdf"),height = 8, width = 5); grid::grid.draw(p1); dev.off()
```



Kassen Plot -- geometric???? all treats
```{r}
my <- myfit500cw[mydwcw$epo %in% c("SALT_A", "SALT_B"),]
my$Treatment <- factor(my$treat, levels = c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80")) # reorder the treat factor
my$id <- paste0(my$epo, my$bc1) # assign bcids
...
my$Fitness_in_CM <- my$mn_fit500_0 # store fitness in complete media
my$Fitness_in_High_Stress <- my$mn_fit500_80 # store fitness in stress
my <- my[!is.na(my$Fitness_in_CM) & !is.na(my$Fitness_in_High_Stress) & my$Treatment %in%  c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"),] # remove NA entries
my$Treatment <- factor(my$Treatment, levels =  c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80")) # reorder the treat factor
...
p1 <- ggplot()+
  ...
  stat_ellipse(data = mypc_elip[mypc_elip$treat %in%  c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, fill = Treatment, color = Treatment), alpha = 0.1, geom = "polygon", linetype = 2, show.legend = F)+
  geom_point(data = myp[myp$treat %in%  c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, color = Treatment), size = 2.5, alpha = 0.35, show.legend = T) +
  geom_point(data = myp[myp$treat %in%  c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, color = Treatment), shape = 1, size = 2.5, alpha = 0.7, show.legend = F)+
  geom_segment(mapping = aes(x=centroids[1,2], y=centroids[1,3], xend=centroids[7,2], yend=centroids[7,3]), linetype=1, size=1.5, inherit.aes=F)+
  geom_point(data=centroids, aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, fill = Treatment),color = "black", cex = 7.5, shape = 21)+
  theme_bw()+
  xlab("Fitness in CM (0% Stress)")+
  ylab("Fitness in 80% Stress")+
  coord_equal()+
  geom_rug(data = myp[myp$treat %in%  c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, color = Treatment))+
  scale_color_manual(values = mypalette)+
  scale_fill_manual(values = mypalette)
p1
pdf(file = paste("000_SALT_KASSEN_0_80_wlines_alltreats.pdf"),height = 8, width = 5); grid::grid.draw(p1); dev.off()
```



Kassen Plot -- geometric???? all treats 0, 80
```{r}
my <- myfit500cw[mydwcw$epo %in% c("SALT_A", "SALT_B"),]
my$Treatment <- factor(my$treat, levels = c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80")) # reorder the treat factor
my$id <- paste0(my$epo, my$bc1) # assign bcids
...
my$fit0dev_0 <- my$mn_fit0_0 - 1
my$fit0dev_80 <- my$mn_fit0_80 - 1
my$Fitness_in_CM <- my$Fitness_in_CM - my$fit0dev_0
my$Fitness_in_High_Stress <- my$Fitness_in_High_Stress - my$fit0dev_80
...
x <- seq(from = min(c(myp$Fitness_in_CM, myp$Fitness_in_High_Stress), na.rm = T),to = max(c(myp$Fitness_in_CM, myp$Fitness_in_High_Stress), na.rm = T), length.out = 500)
y <- seq(from = min(c(myp$Fitness_in_CM, myp$Fitness_in_High_Stress), na.rm = T),to = max(c(myp$Fitness_in_CM, myp$Fitness_in_High_Stress), na.rm = T), length.out = 500)
colnames(geomfit) <- x
rownames(geomfit) <- y
for (i in 1:nrow(geomfit)) {
  for (j in 1:ncol(geomfit)) {
...
p1 <- ggplot()+
  geom_vline(xintercept=1, color = "darkgray")+
  geom_hline(yintercept = 1, color = "darkgray")+
  xlim( min(c(myp$Fitness_in_CM), na.rm = T) - 0.05, max(c(myp$Fitness_in_CM), na.rm = T) + 0.05)+
  ylim( min(c(myp$Fitness_in_High_Stress), na.rm = T) - 0.05, max(c(myp$Fitness_in_High_Stress), na.rm = T) + 0.05)+
  geom_line(data = geomfit[geomfit$geomfitfit == 1,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  geom_line(data = geomfit[geomfit$geomfitfit == 1.05,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  geom_line(data = geomfit[geomfit$geomfitfit == 1.1,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  geom_line(data = geomfit[geomfit$geomfitfit == 1.15,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  ...
  geom_point(data = myp[myp$treat %in%  c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, color = Treatment), shape = 1, size = 2.5, alpha = 0.7, show.legend = F)+
  geom_segment(mapping = aes(x=centroids[1,2], y=centroids[1,3], xend=centroids[7,2], yend=centroids[7,3]), linetype=1, size=1.5, inherit.aes=F)+
  geom_segment(mapping = aes(x=centroids[3,2], y=centroids[3,3], xend=centroids[7,2], yend=centroids[7,3]), linetype=1, size=1.5, inherit.aes=F)+
  geom_segment(mapping = aes(x=centroids[1,2], y=centroids[1,3], xend=centroids[3,2], yend=centroids[3,3]), linetype=1, size=1.5, inherit.aes=F)+
  geom_point(data=centroids, aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, fill = centroids$Treatment),color = "black", cex = 7.5, shape = 21)+
  theme_bw()+
  xlab("Fitness in CM (0% Stress)")+
  ylab("Fitness in 80% Stress")+
  coord_equal()+
...
myp$cm1 <- myp$Fitness_in_CM
myp$st1 <- myp$Fitness_in_High_Stress
myp$cm1_st1 <- (myp$Fitness_in_CM * myp$Fitness_in_High_Stress)^(1/2)
myp$cm2_st1 <- (myp$Fitness_in_CM^2 * myp$Fitness_in_High_Stress)^(1/3)
myp$cm9_st1 <- (myp$Fitness_in_CM^9 * myp$Fitness_in_High_Stress)^(1/10)
myp$cm1_st2 <- (myp$Fitness_in_CM * myp$Fitness_in_High_Stress^2)^(1/3)
myp$cm1_st9 <- (myp$Fitness_in_CM * myp$Fitness_in_High_Stress^9)^(1/10)
mymin <- min(c(myp$cm1, myp$st1, myp$cm1_st1, myp$cm2_st1, myp$cm9_st1, myp$cm1_st2, myp$cm1_st9), na.rm = T)
mymax <- max(c(myp$cm1, myp$st1, myp$cm1_st1, myp$cm2_st1, myp$cm9_st1, myp$cm1_st2, myp$cm1_st9), na.rm = T)
mypalette <- dichrompalette_br_cy
...
p5 <- ggplot()+
  geom_violin(data = myp, aes(x=Treatment, y = cm9_st1))+
  geom_jitter(data = myp, aes(x = Treatment, y = cm9_st1, color = Treatment), width = 0.1, height = 0)+
  coord_flip() + theme_minimal() + scale_color_manual(values = mypalette) + ylab("Fitness") + ggtitle("9xCM : 1xSTRESS") + ylim(c(mymin, mymax)) + theme(legend.position = "none")
# p5
...
p8 <- ggplot()+
  geom_violin(data = myp, aes(x=Treatment, y = cm1_st9))+
  geom_jitter(data = myp, aes(x = Treatment, y = cm1_st9, color = Treatment), width = 0.1, height = 0)+
  coord_flip() + theme_minimal() + scale_color_manual(values = mypalette) + ylab("Fitness") + ggtitle("1xCM : 9xSTRESS") + ylim(c(mymin, mymax)) + theme(legend.position = "none")
# p8
...
pdf(file = paste("000_SALT_GEOMFIT_VIOLINS_00_80.pdf"),height = 11, width = 8.5); grid::grid.draw(x); dev.off()
...
myf <- my # save a frame for limits
my$Fitness_in_CM <- my$mn_fit500_0 # store fitness in complete media
my$Fitness_in_High_Stress <- my$mn_fit500_40 # store fitness in stress

my$fit0dev_0 <- my$mn_fit0_0 - 1
my$fit0dev_40 <- my$mn_fit0_40 - 1
my$Fitness_in_CM <- my$Fitness_in_CM - my$fit0dev_0
my$Fitness_in_High_Stress <- my$Fitness_in_High_Stress - my$fit0dev_40
...
p1 <- ggplot()+
  geom_vline(xintercept=1, color = "darkgray")+
  geom_hline(yintercept = 1, color = "darkgray")+
  xlim( min(c(myp$Fitness_in_CM), na.rm = T) - 0.025, max(c(myp$Fitness_in_CM), na.rm = T) + 0.025)+
  ylim( min(c(myp$Fitness_in_High_Stress), na.rm = T) - 0.025, max(c(myp$Fitness_in_High_Stress), na.rm = T) + 0.025)+
  geom_line(data = geomfit[geomfit$geomfitfit == 1,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  ...
  scale_fill_manual(values = mypalette)
p1
pdf(file = paste("000_SALT_KASSEN_0_40_wlines_alltreats.pdf"),height = 5, width = 8); grid::grid.draw(p1); dev.off()
...
myp$cm2_st1 <- (myp$Fitness_in_CM^2 * myp$Fitness_in_High_Stress)^(1/3)
myp$cm9_st1 <- (myp$Fitness_in_CM^9 * myp$Fitness_in_High_Stress)^(1/10)
myp$cm99_st1 <- (myp$Fitness_in_CM^99 * myp$Fitness_in_High_Stress)^(1/100)

myp$cm1_st2 <- (myp$Fitness_in_CM * myp$Fitness_in_High_Stress^2)^(1/3)
myp$cm1_st9 <- (myp$Fitness_in_CM * myp$Fitness_in_High_Stress^9)^(1/10)
myp$cm1_st99 <- (myp$Fitness_in_CM * myp$Fitness_in_High_Stress^99)^(1/100)
...
pdf(file = paste("000_SALT_GEOMFIT_VIOLINS_00_40.pdf"),height = 11, width = 8.5); grid::grid.draw(x); dev.off()

# which(myp$Treatment == "EH0" & myp$cm1 == min(myp$cm1[myp$Treatment == "EH0"]))
# a <- myp[which(myp$Treatment == "EH0" & myp$cm1 == min(myp$cm1[myp$Treatment == "EH0"])),]
# (a$Fitness_in_CM * a$Fitness_in_High_Stress)^(1/2)
# (a$Fitness_in_CM^2 * a$Fitness_in_High_Stress)^(1/3)
# (a$Fitness_in_CM^9 * a$Fitness_in_High_Stress)^(1/10)



# stats
myp$maxfit <- apply(myp[,c("cm1", "st1")], 1, max, na.rm = T)
myp$minfit <- apply(myp[,c("cm1", "st1")], 1, min, na.rm = T)
myp$varfit <- apply(myp[,c("cm1", "st1")], 1, var, na.rm = T)
myp$meanfit <- apply(myp[,c("cm1", "st1")], 1, mean, na.rm = T)
myp$gain <- apply(myp[,c("cm1", "st1")], 1, sum, na.rm = T); myp$gain <- myp$gain - 2
myp$distance <- abs(myp$cm1 - 1) + abs(myp$st1 - 1)
myp$difference <- myp$maxfit - myp$minfit
myp$arithoverestimation <- myp$meanfit - myp$cm1_st1


summary(lm(cm1_st1 ~ gain, data = myp))
summary(lm(cm1_st1 ~ distance, data = myp))


ggplot()+
  geom_point(aes(y=arithoverestimation, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(I(meanfit - cm1_st1) ~ st1, data = myp))

ggplot()+
  geom_point(aes(y=minfit, x=maxfit, color = Treatment), data = myp)+
  geom_vline(xintercept = 1) + geom_hline(yintercept = 1)+
  scale_color_manual(values = mypalette); summary(lm(minfit ~ maxfit, data = myp))

ggplot()+
  geom_point(aes(y=gain, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(I(meanfit - cm1_st1) ~ st1, data = myp))

ggplot()+
  geom_point(aes(y=distance, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(I(meanfit - cm1_st1) ~ st1, data = myp))

ggplot()+
  geom_point(aes(y=gain, x=distance, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(gain ~ distance, data = myp))
...
ggplot()+
  geom_point(aes(y=cm1_st1, x=difference, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm2_st1, x=difference, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm2_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm9_st1, x=difference, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm9_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm1, x=difference, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1 ~ st1, data = myp))




ggplot()+
  geom_point(aes(y=cm1_st1, x=gain, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm2_st1, x=gain, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm2_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm9_st1, x=gain, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm9_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm1, x=gain, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1 ~ st1, data = myp))



ggplot()+
  geom_point(aes(y=cm1_st1, x=distance, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm2_st1, x=distance, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm2_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm9_st1, x=distance, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm9_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm1, x=distance, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1 ~ st1, data = myp))
...
ggplot()+
  geom_point(aes(y=cm1_st1, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm2_st1, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm2_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm9_st1, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm9_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm1, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1 ~ st1, data = myp))

ggplot()+
  geom_point(aes(y=cm1_st1, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st1 ~ cm1, data = myp))
ggplot()+
  geom_point(aes(y=cm1_st2, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st2 ~ cm1, data = myp))
ggplot()+
  geom_point(aes(y=cm1_st9, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st9 ~ cm1, data = myp))
ggplot()+
  geom_point(aes(y=st1, x=maxfit, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(st1 ~ cm1, data = myp))
...
ggplot()+
  geom_point(aes(y=cm1_st1, x=st1, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm2_st1, x=st1, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm2_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm9_st1, x=st1, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm9_st1 ~ st1, data = myp))
ggplot()+
  geom_point(aes(y=cm1, x=st1, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1 ~ st1, data = myp))

ggplot()+
  geom_point(aes(y=cm1_st1, x=cm1, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st1 ~ cm1, data = myp))
ggplot()+
  geom_point(aes(y=cm1_st2, x=cm1, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st2 ~ cm1, data = myp))
ggplot()+
  geom_point(aes(y=cm1_st9, x=cm1, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(cm1_st9 ~ cm1, data = myp))
ggplot()+
  geom_point(aes(y=st1, x=cm1, color = Treatment), data = myp)+
  scale_color_manual(values = mypalette); summary(lm(st1 ~ cm1, data = myp))

```

Kassen Plot -- geometric???? all treats 40, 80
```{r}
my <- myfit500cw[mydwcw$epo %in% c("SALT_A", "SALT_B"),]
my$Treatment <- factor(my$treat, levels = c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80")) # reorder the treat factor
my$id <- paste0(my$epo, my$bc1) # assign bcids
myf <- my # save a frame for limits
my$Fitness_in_CM <- my$mn_fit500_40 # store fitness in complete media
my$Fitness_in_High_Stress <- my$mn_fit500_80 # store fitness in stress

my$fit0dev_40 <- my$mn_fit0_40 - 1
my$fit0dev_80 <- my$mn_fit0_80 - 1
my$Fitness_in_CM <- my$Fitness_in_CM - my$fit0dev_40
my$Fitness_in_High_Stress <- my$Fitness_in_High_Stress - my$fit0dev_80
...
# work on geometric fitness lines
geomfit <- matrix(nrow= 100, ncol = 100)
x <- seq(from = min(c(myp$Fitness_in_CM, myp$Fitness_in_High_Stress), na.rm = T),to = max(c(myp$Fitness_in_CM, myp$Fitness_in_High_Stress), na.rm = T), length.out = 100)
y <- seq(from = min(c(myp$Fitness_in_CM, myp$Fitness_in_High_Stress), na.rm = T),to = max(c(myp$Fitness_in_CM, myp$Fitness_in_High_Stress), na.rm = T), length.out = 100)
colnames(geomfit) <- x
rownames(geomfit) <- y
for (i in 1:nrow(geomfit)) {
  for (j in 1:ncol(geomfit)) {
...
p1 <- ggplot()+
  ...
  scale_fill_manual(values = mypalette)
p1
pdf(file = paste("000_SALT_KASSEN_40_80_wlines_alltreats.pdf"),height = 8, width = 5); grid::grid.draw(p1); dev.off()
```


Kassen Plot -- geometric???? all treats 0, 120
```{r}
my <- myfit500cw[mydwcw$epo %in% c("SALT_A", "SALT_B"),]
my$Treatment <- factor(my$treat, levels = c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80")) # reorder the treat factor
my$id <- paste0(my$epo, my$bc1) # assign bcids
myf <- my # save a frame for limits
my$Fitness_in_CM <- my$mn_fit500_0 # store fitness in complete media
my$Fitness_in_High_Stress <- my$mn_fit500_120 # store fitness in stress

my$fit0dev_0 <- my$mn_fit0_0 - 1
my$fit0dev_120 <- my$mn_fit0_120 - 1
my$Fitness_in_CM <- my$Fitness_in_CM - my$fit0dev_0
my$Fitness_in_High_Stress <- my$Fitness_in_High_Stress - my$fit0dev_120
...
p1 <- ggplot()+
  ...
  geom_hline(yintercept = 1, color = "darkgray")+
  xlim( min(c(myp$Fitness_in_CM), na.rm = T) - 0.05, max(c(myp$Fitness_in_CM), na.rm = T) + 0.05)+
  ylim( min(c(myp$Fitness_in_High_Stress), na.rm = T) - 0.15, max(c(myp$Fitness_in_High_Stress), na.rm = T) + 0.15)+
  geom_line(data = geomfit[geomfit$geomfitfit == 1,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  geom_line(data = geomfit[geomfit$geomfitfit == 1.05,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  geom_line(data = geomfit[geomfit$geomfitfit == 1.1,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  geom_line(data = geomfit[geomfit$geomfitfit == 1.15,], aes(x = geomfitx, y = geomfity), lty  = "dotted")+
  ...
  theme_bw()+
  xlab("Fitness in CM (0% Stress)")+
  ylab("Fitness in 120% Stress")+
  coord_equal()+
  geom_rug(data = myp[myp$treat %in%  c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, color = Treatment))+
  scale_color_manual(values = mypalette)+
  scale_fill_manual(values = mypalette)
p1
pdf(file = paste("000_SALT_KASSEN_0_120_wlines_alltreats.pdf"),height = 8, width = 5); grid::grid.draw(p1); dev.off()
```




Kassen Plot -- geometric???? all treats 40, 120
```{r}
my <- myfit500cw[mydwcw$epo %in% c("SALT_A", "SALT_B"),]
my$Treatment <- factor(my$treat, levels = c("EH0", "EH0_40", "EH40", "EH20_60", "EH0_80", "EH40_80", "EH80")) # reorder the treat factor
my$id <- paste0(my$epo, my$bc1) # assign bcids
...
p1 <- ggplot()+
  ...
  scale_fill_manual(values = mypalette)
p1
pdf(file = paste("000_SALT_KASSEN_40_120_wlines_alltreats.pdf"),height = 8, width = 5); grid::grid.draw(p1); dev.off()
```


KASSEN PLOT -- CURRENT VERSION -- SHARED WITH JF!
```{r}
# for SALT  ---------------------------------------------------------------------
#  0, 80 ------------------------------------------------------------------------
my <- mydwcw[mydwcw$epo %in% c("SALT_A", "SALT_B"),]
...
p1 <- ggplot()+
  ...
  geom_point(data = myp[myp$treat %in% c("EH0","EH0_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress, color = Treatment), size = 2.5, alpha = 0.5, show.legend = T) +
  geom_point(data = myp[myp$treat %in% c("EH0","EH0_80", "EH80"),], aes(x=Fitness_in_CM, y=Fitness_in_High_Stress), shape = 1, colour = "black", size = 2.5, alpha = 0.7, show.legend = F)+
  geom_segment(mapping = aes(x=centroids_uw[1,2], y=centroids_uw[1,3], xend=centroids_uw[3,2], yend=centroids_uw[3,3]), linetype=1, size=1.5, inherit.aes=F)+
  geom_point(data=centroids_uw, aes(x=Fitness_in_CM, y=Fitness_in_High_Stress), color = "black", cex = 5, shape = 21, fill = "black")+
  theme_bw()+
  xlab("Fitness in CM (0% Stress)")+
  ylab("Fitness in 80% Stress")+
  coord_equal()+
  ...
  scale_fill_manual(values = mypalette)
p1
pdf(file = paste("000_SALT_KASSEN_0_80.pdf"),height = 8, width = 5); grid::grid.draw(p1); dev.off()
...
myp$cutoff <- NA; myp$cutoff_elip <- NA
myp$md <- NA
for (i in c("EH40", "EH40_80","EH80")) {
  myt <- myp[myp$Treatment == i,]
  md <- mahalanobis(x = myt[, c("Fitness_in_Low_Stress", "Fitness_in_High_Stress")], 
                  center = colMeans(myt[, c("Fitness_in_Low_Stress", "Fitness_in_High_Stress")]), 
                  cov = cov(myt[, c("Fitness_in_Low_Stress", "Fitness_in_High_Stress")]))
  myp[myp$Treatment == i,]$md <- md
  alpha_elip <- 0.25
  alpha <- 0.05
  cutoff <- (qchisq(p = 1 - alpha, df = ncol(myp[myp$Treatment == i,][, c("Fitness_in_Low_Stress", "Fitness_in_High_Stress")])))
  cutoff_elip <- (qchisq(p = 1 - alpha_elip, df = ncol(myp[myp$Treatment == i,][, c("Fitness_in_Low_Stress", "Fitness_in_High_Stress")])))
...
# viz
plot(myp$Fitness_in_High_Stress ~ myp$Fitness_in_Low_Stress)
points(myp[myp$md >= myp$cutoff,]$Fitness_in_High_Stress ~ myp[myp$md >= myp$cutoff,]$Fitness_in_Low_Stress, col = "red")
points(myp[myp$md >= myp$cutoff_elip,]$Fitness_in_High_Stress ~ myp[myp$md >= myp$cutoff_elip,]$Fitness_in_Low_Stress, col = "blue")

# Calculate unweighted and weighted centroids dataframes --------
mypc <- myp[myp$md < myp$cutoff,]
centroids <- aggregate(cbind(mypc$Fitness_in_Low_Stress, mypc$Fitness_in_High_Stress)~mypc$Treatment,mypc,mean)
...
p2 <- ggplot()+
  ...
  xlim( min(c(myf$mn_dw_0, myf$mn_dw_40), na.rm = T), max(c(myf$mn_dw_0, myf$mn_dw_40), na.rm = T))+
  ylim( min(c(myf$mn_dw_40, myf$mn_dw_80), na.rm = T), max(c(myf$mn_dw_40, myf$mn_dw_80), na.rm = T))+
  stat_ellipse(data = mypc_elip[mypc_elip$treat %in% c("EH40","EH40_80", "EH80"),], aes(x=Fitness_in_Low_Stress, y=Fitness_in_High_Stress, fill = Treatment, color = Treatment), alpha = 0.15, geom = "polygon", linetype = 2, show.legend = F)+
  ...
  geom_point(data = myp[myp$treat %in% c("EH40","EH40_80", "EH80"),], aes(x=Fitness_in_Low_Stress, y=Fitness_in_High_Stress), shape = 1, colour = "black", size = 2.5, alpha = 0.7, show.legend = F)+
  geom_segment(mapping = aes(x=centroids_uw[1,2], y=centroids_uw[1,3], xend=centroids_uw[3,2], yend=centroids_uw[3,3]), linetype=1, size=1.5, inherit.aes=F)+
  geom_point(data=centroids_uw, aes(x=Fitness_in_Low_Stress, y=Fitness_in_High_Stress), color = "black", cex = 5, shape = 21, fill = "black")+
  theme_bw()+
  xlab("Fitness in 40% Stress")+
  ylab("Fitness in 80% Stress")+
  coord_equal()+
  ...
  scale_fill_manual(values = mypalette)
p2
pdf(file = paste("000_SALT_KASSEN_40_80.pdf"),height = 8, width = 5); grid::grid.draw(p2); dev.off()
...
myp$cutoff <- NA; myp$cutoff_elip <- NA
myp$md <- NA
for (i in c("EH0", "EH0_40", "EH40")) {
  myt <- myp[myp$Treatment == i,]
  md <- mahalanobis(x = myt[, c("Fitness_in_CM", "Fitness_in_Low_Stress")], 
                  center = colMeans(myt[, c("Fitness_in_CM", "Fitness_in_Low_Stress")]), 
                  cov = cov(myt[, c("Fitness_in_CM", "Fitness_in_Low_Stress")]))
  myp[myp$Treatment == i,]$md <- md
  alpha_elip <- 0.25
  alpha <- 0.05
  cutoff <- (qchisq(p = 1 - alpha, df = ncol(myp[myp$Treatment == i,][, c("Fitness_in_CM", "Fitness_in_Low_Stress")])))
  cutoff_elip <- (qchisq(p = 1 - alpha_elip, df = ncol(myp[myp$Treatment == i,][, c("Fitness_in_CM", "Fitness_in_Low_Stress")])))
...
# viz
plot(myp$Fitness_in_Low_Stress ~ myp$Fitness_in_CM)
points(myp[myp$md >= myp$cutoff,]$Fitness_in_Low_Stress ~ myp[myp$md >= myp$cutoff,]$Fitness_in_CM, col = "red")
points(myp[myp$md >= myp$cutoff_elip,]$Fitness_in_Low_Stress ~ myp[myp$md >= myp$cutoff_elip,]$Fitness_in_CM, col = "blue")

# Calculate unweighted and weighted centroids dataframes --------
mypc <- myp[myp$md < myp$cutoff,]
centroids <- aggregate(cbind(mypc$Fitness_in_CM, mypc$Fitness_in_Low_Stress)~mypc$Treatment,mypc,mean)
...
p3 <- ggplot()+
  ...
  xlim( min(c(myf$mn_dw_0, myf$mn_dw_40), na.rm = T), max(c(myf$mn_dw_0, myf$mn_dw_40), na.rm = T))+
  ylim( min(c(myf$mn_dw_40, myf$mn_dw_80), na.rm = T), max(c(myf$mn_dw_40, myf$mn_dw_80), na.rm = T))+
  stat_ellipse(data = mypc_elip[mypc_elip$treat %in% c("EH0", "EH0_40", "EH40"),], aes(x=Fitness_in_CM, y=Fitness_in_Low_Stress, fill = Treatment, color = Treatment), alpha = 0.15, geom = "polygon", linetype = 2, show.legend = F)+
  ...
  geom_point(data = myp[myp$treat %in% c("EH0", "EH0_40", "EH40"),], aes(x=Fitness_in_CM, y=Fitness_in_Low_Stress), shape = 1, colour = "black", size = 2.5, alpha = 0.7, show.legend = F)+
  geom_segment(mapping = aes(x=centroids_uw[1,2], y=centroids_uw[1,3], xend=centroids_uw[3,2], yend=centroids_uw[3,3]), linetype=1, size=1.5, inherit.aes=F)+
  geom_point(data=centroids_uw, aes(x=Fitness_in_CM, y=Fitness_in_Low_Stress), color = "black", cex = 5, shape = 21, fill = "black")+
  theme_bw()+
  xlab("Fitness in CM (0% Stress)")+
  ylab("Fitness in 40% Stress")+
  coord_equal()+
  ...
  scale_fill_manual(values = mypalette)
p3
pdf(file = paste("000_SALT_KASSEN_0_40.pdf"),height = 8, width = 5); grid::grid.draw(p3); dev.off()
...
p1 <- ggplot()+
  ...
  scale_fill_manual(values = mypalette)
p1
pdf(file = paste("000_SALT_KASSEN_0_80_us.pdf"),height = 8, width = 5); grid::grid.draw(p1); dev.off()
...
p2 <- ggplot()+
  ...
  scale_fill_manual(values = mypalette)
p2
pdf(file = paste("000_SALT_KASSEN_40_80_us.pdf"),height = 8, width = 5); grid::grid.draw(p2); dev.off()
...
p3 <- ggplot()+
  ...
  scale_fill_manual(values = mypalette)
p3
pdf(file = paste("000_SALT_KASSEN_0_40_us.pdf"),height = 8, width = 5); grid::grid.draw(p3); dev.off()
