...
# create the frame, omitting most cols not necessary for these upcoming plots and tests.
my <- myd
my <- my[,c(1:7, 83:88, 111, 122, 126)] # reatain only interst columns
my1 <- my[my$mpar == 1,] # r1
my2 <- my[my$mpar == 2,]; colnames(my2) <- paste0(colnames(my2), "_2") # r2
my3 <- my[my$mpar == 3,]; colnames(my3) <- paste0(colnames(my3), "_3") # r3
my4 <- my[my$mpar == 4,]; colnames(my4) <- paste0(colnames(my4), "_4") # r4
...
my <- cbind(my0, my40$mn_fit500, my40$mn_re_fit500, my80$mn_fit500, my80$mn_re_fit500, my120$mn_fit500, my120$mn_re_fit500)
colnames(my)[c(23:30)] <- c("mn_fit500_0", "mn_re_fit500_0","mn_fit500_40", "mn_re_fit500_40", "mn_fit500_80", "mn_re_fit500_80", "mn_fit500_120", "mn_re_fit500_120")
myfit500cw <- my
...
MN VS VAR FITNESS (2 ENVIRONMENT) -- BASIC
```{r}
...
MN VS VAR FITNESS (2 ENVIRONMENT) -- ARITHMETIC &&&&& GEOMETRIC (NOT CURRENTLY WORKING!!!)
```{r}
...
mydata$mn_fit500_0_80uwg <- apply(mydata, 1, function(x) psych::geometric.mean(x=as.numeric(x[c( "mn_fit500_0", "mn_fit500_80")]), na.rm = F)) # I checked this math, its good.
...
# mypalette <- dichrompalette_br_cy
# mydata$mn_fit500_0_80s <- scale(mydata$mn_fit500_0_80uw, center = T); mydata$var_fit500_0_80s <- scale(mydata$var_fit500_0_80uw, center = T)
# mydat <- mydata[!is.na(mydata$var_fit500_0_80s) & !is.na(mydata$mn_fit500_0_80s),]
# fit<-lm(var_fit500_0_80s ~ mn_fit500_0_80s, data =mydat)
# mydat$predicted <- predict(fit)
# mydat$residuals <- residuals(fit)
# ggplot(mydat, aes(x = mn_fit500_0_80s, y = var_fit500_0_80s)) +
#   geom_smooth(method = "lm", se = FALSE, color = "lightgrey") +
#   geom_segment(aes(xend = mn_fit500_0_80s, yend = predicted), alpha = .2) +
# 
#   # > Color adjustments made here...
#   geom_point(aes(color = Treatment)) +  # Color mapped here
#   scale_color_manual(values = mypalette)+
#   guides(color = FALSE) +
# 
#   # <
#   theme_bw()
# tapply(mydat$residuals, mydat$Treatment, mean)
# 
# 
# 
# mypalette <- dichrompalette_br_cy
# 
# mydata$x <-  scale(mydata$mn_fit500_0_80uw, center = T)
# mydata$y <-  scale(mydata$var_fit500_0_80uw, center = T)
# mydat <- mydata[!is.na(mydata$x) & !is.na(mydata$y),]
# myslope <- 1
# myintercept <- mean(mydat$x)
# mydat$predicted1to1 <- mydat$x*myslope + myintercept
# mydat$residuals <- mydat$y - mydat$predicted1to1
# ggplot(mydat, aes(x = x, y = y)) +
#   geom_segment(aes(xend = x, yend = predicted1to1), alpha = .2) +
#   geom_abline(slope =1, intercept = 0, linetype = "dashed", color = "gray75")+
# 
#   # > Color adjustments made here...
#   geom_point(aes(color = Treatment)) +  # Color mapped here
...
mypalette <- dichrompalette_br_cy
mydata$mn_fit500_0_80gs <- scale(mydata$mn_fit500_0_80uwg, center = T); mydata$var_fit500_0_80s <- scale(mydata$var_fit500_0_80uw, center = T)
p <- ggplot(mydata, aes(x=mn_fit500_0_80gs, y=var_fit500_0_80s, color = Treatment, fill = Treatment)) + 
  geom_abline(slope =1, intercept = 0, linetype = "dashed", color = "gray75")+
  geom_point(size = 2.5, alpha = 0.75) + 
  scale_color_manual(values = mypalette)+
  theme_bw()+
...
# weighted
mypalette <- dichrompalette_br_cy
mydata$mn_fit500_0_80s <- scale(mydata$mn_fit500_0_80, center = T); mydata$var_fit500_0_80s <- scale(mydata$var_fit500_0_80, center = T)
p <- ggplot(mydata, aes(x=mn_fit500_0_80s, y=var_fit500_0_80s, color = Treatment, fill = Treatment)) + 
  geom_abline(slope =1, intercept = 0, linetype = "dashed", color = "gray75")+
  geom_point(size = 2.5, alpha = 0.75) + 
  scale_color_manual(values = mypalette)+
...
mypalette <- dichrompalette_br_cy
mydata$mn_fit500_0_80gs <- scale(mydata$mn_fit500_0_80g, center = T); mydata$var_fit500_0_80s <- scale(mydata$var_fit500_0_80, center = T)
p <- ggplot(mydata, aes(x=mn_fit500_0_80gs, y=var_fit500_0_80s, color = Treatment, fill = Treatment)) + 
  geom_abline(slope =1, intercept = 0, linetype = "dashed", color = "gray75")+
  geom_point(size = 2.5, alpha = 0.75) + 
  scale_color_manual(values = mypalette)+
...
MN FITNESS (2 ENVIRONMENT) -- ARITHMETIC &&&&& GEOMETRIC (NOT CURRENTLY WORKING!!!)
```{r}
...
# weighted -------------------
mypalette <- dichrompalette_br_cy
mydata$mn_fit500_0_80s <- scale(mydata$mn_fit500_0_80, center = T); mydata$mn_fit500_0_80gs <- scale(mydata$mn_fit500_0_80g, center = T)
p <- ggplot(mydata, aes(x=mn_fit500_0_80s, y=mn_fit500_0_80gs, color = Treatment, fill = Treatment)) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray75")+
  geom_point(size = 2.5, alpha = 0.75) + 
  scale_color_manual(values = mypalette)+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  xlab("Mean Fitness ARI: CM (No Stress), 80% Stress")+
  ylab("Mean Fitness GEO: CM (No Stress), 80% Stress")+
  geom_rug()
p1 <- p + facet_wrap(~treat, nrow = 2) + ylab("")
p2 <- p + theme(legend.position = "none") + 
  geom_text(aes(x=3,y=3), color = "gray75", label = "1:1")
...
# unweighted -------------------
mypalette <- dichrompalette_br_cy
mydata$mn_fit500_0_80s <- scale(mydata$mn_fit500_0_80uw, center = T); mydata$mn_fit500_0_80gs <- scale(mydata$mn_fit500_0_80uwg, center = T)
p <- ggplot(mydata, aes(x=mn_fit500_0_80s, y=mn_fit500_0_80gs, color = Treatment, fill = Treatment)) + 
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray75")+
  geom_point(size = 2.5, alpha = 0.75) + 
  scale_color_manual(values = mypalette)+
...
moddata <- mydata[!is.na(mydata$mn_fit500_0_80uwg) & !is.na(mydata$mn_fit500_0_80uw),]
mymod <- glm(moddata$mn_fit500_0_80uwg ~ moddata$mn_fit500_0_80uw)
moddata$resid <- residuals(mymod)
tapply(moddata$resid, moddata$Treatment, mean)

# weighted
mypalette <- dichrompalette_br_cy
myx <- mydata$Treatment
myy <- mydata$mn_fit500_0_80
...
x <- glm(mydata$mn_fit500_0_80uw ~ mydata$mn_fit500_0_80uwg)
plot(sort(residuals(x)));abline(h=0, lty = 1, lwd = 2, col = "red")
hist(residuals(x), breaks = 100);abline(v=0, lty = 1, lwd = 2, col = "red")
summary(residuals(x))
```
